...
# Base64-encoded PKCS#8 PEM private key generated by scripts/oauth/generate-bootstrap.mjs
OAUTH_JWT_PRIVATE_KEY=base64_encoded_pkcs8_private_key
# Deterministic kid generated from the public JWK thumbprint
OAUTH_JWT_KEY_ID=generated_jwk_thumbprint
# Issuer used in access tokens and ID tokens
...
### OAuth Bootstrap Material
Run `node scripts/oauth/generate-bootstrap.mjs` to generate local OAuth bootstrap artifacts.

- `.env.oauth.local`: gitignored local secrets file containing the private signing key, `kid`, and the plaintext CMS client secrets.
- `supabase/.temp/oauth-bootstrap.public.json`: gitignored non-sensitive artifact containing the public JWK and client metadata for reference.
- `supabase/.temp/oauth-bootstrap.sql`: gitignored SQL file that rotates the seeded OAuth clients to the generated secrets and inserts the active signing key row into `public.oauth_signing_keys`.
...
Notes:
- `OAUTH_JWT_PRIVATE_KEY` is written as a base64-encoded PKCS#8 PEM string to avoid multiline secret handling issues in Edge Functions.
- The migration seeds the OAuth clients with secure placeholder bcrypt hashes using database-generated random material. Apply `supabase/.temp/oauth-bootstrap.sql` after running the generator so the stored hashes match the local plaintext client secrets.
- The default production redirect URI emitted by the generator is `https://cms.invalid/api/auth/crm/callback` and must be replaced with the real CMS callback URI before production rollout.

### OIDC Discovery And JWKS
The OAuth authorization server publishes public metadata through Supabase Edge Function paths:

- Discovery document: `https://<project-ref>.supabase.co/functions/v1/oauth-discovery`
- JWKS document: `https://<project-ref>.supabase.co/functions/v1/oauth-jwks`

If a custom domain or reverse proxy fronts the authorization server, alias the canonical OIDC discovery path `/.well-known/openid-configuration` to `/functions/v1/oauth-discovery`. The discovery document itself publishes the current JWKS endpoint at `/functions/v1/oauth-jwks`.
