CREATE OR REPLACE FUNCTION public.get_activity_feed(
  ...
  p_integration_names TEXT[] DEFAULT NULL,
...
BEGIN
  ...
  new_events AS (
    ...
    WHERE e.tenant_id = v_tenant_id
      ...
      AND (
        ...
        OR e.customer_id IN (SELECT bc.id FROM base_customers bc)
      )
  ),
  legacy_timeline_events AS (
    ...
    WHERE c.tenant_id = v_tenant_id
      AND l.customer_id IN (SELECT bc.id FROM base_customers bc)
  ...
  legacy_timeline AS (
    ...
    WHERE c.tenant_id = v_tenant_id
      AND t.customer_id IN (SELECT bc.id FROM base_customers bc)
  ...
  FROM merged m
  WHERE (
    p_integration_names IS NULL OR
    array_length(p_integration_names, 1) IS NULL OR
    m.integration_name = ANY(p_integration_names)
  )
...
GRANT EXECUTE ON FUNCTION public.get_activity_feed(UUID, INT, INT, TEXT, TEXT[], TEXT[], TEXT[], TEXT[], TEXT[], TIMESTAMPTZ, TIMESTAMPTZ, UUID[], TEXT[]) TO authenticated;