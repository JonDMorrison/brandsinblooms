...
BEGIN
	...
	persona_assignments AS (
		...
		FROM (
			SELECT spc.id AS customer_id, custom_persona.id::TEXT AS persona_id
			FROM scoped_persona_customers spc
			JOIN public.customer_personas cp ON cp.customer_id = spc.id
			JOIN public.crm_personas custom_persona ON custom_persona.id = cp.persona_id
			WHERE cp.persona_id IS NOT NULL
				AND (
					(
						p_tenant_id IS NOT NULL
						AND custom_persona.tenant_id = p_tenant_id
					)
					OR (
						p_tenant_id IS NULL
						AND p_user_id IS NOT NULL
						AND custom_persona.user_id = p_user_id
					)
				)

			UNION ALL

			SELECT spc.id AS customer_id, system_persona.id AS persona_id
			FROM scoped_persona_customers spc
			JOIN public.customer_personas cp ON cp.customer_id = spc.id
			JOIN system_personas system_persona ON system_persona.id = cp.predefined_persona_id
			WHERE cp.predefined_persona_id IS NOT NULL

			UNION ALL

			SELECT spc.id AS customer_id, custom_persona.id::TEXT AS persona_id
			FROM scoped_persona_customers spc
			JOIN public.crm_personas custom_persona ON custom_persona.id = spc.persona_id
			WHERE spc.persona_id IS NOT NULL
				AND (
					(
						p_tenant_id IS NOT NULL
						AND custom_persona.tenant_id = p_tenant_id
