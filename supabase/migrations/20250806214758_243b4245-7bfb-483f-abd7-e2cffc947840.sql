...
SELECT 
  ...
  CASE 
    WHEN COALESCE(order_stats.total_spent, 0) >= 1000 THEN 'VIP'
    WHEN COALESCE(order_stats.total_spent, 0) >= 500 THEN 'Loyal'
    WHEN COALESCE(order_stats.total_spent, 0) >= 100 THEN 'Regular'
    ELSE 'New'
...
LEFT JOIN (
  ...
  FROM pos_orders po
  JOIN pos_customers pc ON pc.external_id = po.external_customer_id
  CROSS JOIN LATERAL jsonb_array_elements(po.items) AS item
  GROUP BY pc.external_customer_id
) order_stats ON order_stats.external_customer_id = c.external_id;
...
-- Create function for updating segments timestamp
