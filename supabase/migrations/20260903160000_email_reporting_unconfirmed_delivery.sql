-- Distinguish provider acceptance from confirmed delivery disposition. A
-- campaign is not delivery-complete while accepted recipients lack a delivered,
-- bounced, failed, skipped, or pending outcome.
  ...
  recipient_state AS (
    SELECT
      ...
      state.sent
        ...
        AND NOT state.clicked
        AND NOT state.bounced
        AND NOT state.complained
        AND NOT state.unsubscribed
        AND NOT state.failed
        AND NOT state.skipped
        AND NOT state.pending AS unconfirmed
    FROM recipient_state_base AS state
  ),
  totals AS (
    SELECT
      ...
      count(*) FILTER (WHERE unconfirmed)::bigint AS unconfirmed
  ...
  SELECT jsonb_build_object(
    'metrics', jsonb_build_object(
      ...
      'diagnostics', jsonb_build_object(
        ...
        'delivery_complete', totals.pending = 0 AND totals.unconfirmed = 0,
        'unconfirmed_delivery', totals.unconfirmed,
...
COMMENT ON FUNCTION public.get_email_campaign_reporting_snapshot(uuid) IS
  'Returns an uncapped tenant-authorized email campaign delivery, engagement, failure, unconfirmed disposition, and 72-hour timeline snapshot.';
