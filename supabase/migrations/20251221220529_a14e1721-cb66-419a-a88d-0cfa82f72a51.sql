...
BEGIN
  ...
  WITH unified_events AS (
    ...
    FROM pos_orders o
    JOIN crm_customers c ON c.clover_customer_id = o.pos_customer_id
    ...
    SELECT 
      s.id,
      CASE 
        WHEN s.direction = 'outbound' THEN 'sms_sent'
        ELSE 'sms_received'
      END as event_type,
      'sms'::TEXT as event_category,
      CASE 
        WHEN s.direction = 'outbound' THEN 'SMS Sent'
        ELSE 'SMS Received'
      END as title,
      LEFT(s.body, 100) as description,
      'neutral'::TEXT as impact,
      jsonb_build_object(
        'direction', s.direction,
        'status', s.status,
        'body_preview', LEFT(s.body, 200)
      ) as metadata,
      s.created_at
    FROM sms_messages s
