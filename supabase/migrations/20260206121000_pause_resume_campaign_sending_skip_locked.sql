-- Reduce pause/resume timeouts by avoiding lock waits on actively processed rows.
-- Use FOR UPDATE SKIP LOCKED so we don't block behind the worker.
...
BEGIN
  ...
  -- Pause unsent messages, but don't wait for locks held by the worker.
  WITH to_pause AS (
    SELECT m.ctid
  ...
  -- Pause jobs so the worker doesn't claim them, without waiting on locks.
  WITH to_pause AS (
    SELECT j.ctid
...
BEGIN
  ...
  -- Resume paused messages without waiting on locks.
  WITH to_resume AS (
    SELECT m.ctid
    FROM public.email_messages m
    WHERE m.campaign_id = p_campaign_id
  ...
  -- Resume paused jobs without waiting on locks.
  WITH to_resume AS (
    SELECT j.ctid
    FROM public.email_send_jobs j
    WHERE j.campaign_id = p_campaign_id
