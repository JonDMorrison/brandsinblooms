...
-- Fix the admin_delete_user function to handle foreign key constraints properly
...
BEGIN
  ...
  -- Delete content_tasks FIRST (since it references campaigns and users)
  DELETE FROM public.content_tasks WHERE user_id = target_user_id;
  DELETE FROM public.content_tasks WHERE created_by_user_id = target_user_id;
  
  -- Now we can safely delete campaigns
  DELETE FROM public.campaigns WHERE user_id = target_user_id;
  DELETE FROM public.campaigns WHERE created_by_user_id = target_user_id;
  ...
  DELETE FROM public.analytics_data WHERE connection_id IN (
    SELECT id FROM public.social_connections WHERE user_id = target_user_id
  );
  DELETE FROM public.team_members WHERE user_id = target_user_id;
  DELETE FROM public.team_members WHERE invited_by = target_user_id;
  ...
  DELETE FROM public.image_suggestions WHERE content_task_id IN (
    SELECT id FROM public.content_tasks WHERE user_id = target_user_id
  );
  DELETE FROM public.post_performance WHERE content_task_id IN (
    SELECT id FROM public.content_tasks WHERE user_id = target_user_id
  );
