-- Make get_email_campaign_progress resilient to tenant_id/user_id drift.
-- Problem: the UI uses this RPC for Emails / Last Activity. Previous versions returned no rows when
-- - public.users.tenant_id was NULL for the actor, or
-- - crm_campaigns.tenant_id was NULL/mismatched, even though email_messages existed.
--
-- Solution: authorize by owner OR same-tenant, and derive an effective tenant_id from campaign links.
...
-- Reload PostgREST schema cache so the function is immediately discoverable.
