...
-- Repair migration: restore admin_set_campaign_intervention_state RPC(s)
-- Needed by /admin/tenants/:id/email campaign interventions.

DROP FUNCTION IF EXISTS public.admin_set_campaign_intervention_state(UUID, BOOLEAN, BOOLEAN, BOOLEAN, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.admin_set_campaign_intervention_state(UUID, BOOLEAN, BOOLEAN, BOOLEAN, TEXT, TEXT, TIMESTAMPTZ);
...
-- 6-arg compatibility overload (the UI error references this signature)
CREATE OR REPLACE FUNCTION public.admin_set_campaign_intervention_state(
  p_campaign_id UUID,
  p_admin_paused BOOLEAN,
  p_force_stopped BOOLEAN,
...
AS $$
BEGIN
  RETURN public.admin_set_campaign_intervention_state(
    p_campaign_id,
    p_admin_paused,
    p_force_stopped,
    p_autopause_override_enabled,
    p_autopause_override_precedence,
    p_reason,
    NULL
  );
END;
