-- Fix: apply_email_send_results compares email_messages.claim_token (UUID) to p_claim_token (TEXT)
-- which can raise: operator does not exist: uuid = text.
--
-- Keep p_claim_token as TEXT to avoid PostgREST schema cache signature mismatch,
-- but cast once into a UUID and use that for comparisons.
...
BEGIN
  ...
  BEGIN
    v_claim_token := p_claim_token::uuid;
  EXCEPTION
  ...
  u AS (
    ...
    FROM r
    WHERE m.id = r.msg_id
      AND m.claim_token = v_claim_token
