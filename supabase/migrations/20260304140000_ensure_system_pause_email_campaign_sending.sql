-- Ensure the system-level pause RPC exists (service-role / internal callers).
-- Some environments may have applied user-level pause without the system RPC,
-- which breaks edge functions that need to pause sending when governance blocks.
...
REVOKE ALL ON FUNCTION public.system_pause_email_campaign_sending(UUID, TEXT, TEXT) FROM PUBLIC;
