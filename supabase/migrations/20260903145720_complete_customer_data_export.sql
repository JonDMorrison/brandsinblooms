-- Export every active customer without relying on PostgREST's row cap. The
-- browser receives one bounded JSON page at a time and follows an opaque UUID
-- cursor until the tenant export is complete.

CREATE INDEX IF NOT EXISTS idx_crm_customers_active_tenant_export
ON public.crm_customers (tenant_id, id)
...
BEGIN
  ...
  v_is_master_admin := coalesce(public.is_master_admin(v_user_id), false);
  ...
  ELSE
    ...
    IF v_user_role IS DISTINCT FROM 'admin' THEN
      RAISE EXCEPTION 'Customer exports require administrator access'
        USING ERRCODE = '42501';
    END IF;
  END IF;
  ...
  SELECT jsonb_build_object(
    ...
    'nextCursor', (SELECT cursor_row.id FROM export_rows AS cursor_row ORDER BY cursor_row.id DESC LIMIT 1),
...
COMMENT ON FUNCTION public.get_customer_export_page(uuid, integer) IS
  'Returns one tenant-authorized, keyset-paginated customer export page including custom fields, segments, consent, POS summaries, and loyalty balances.';
