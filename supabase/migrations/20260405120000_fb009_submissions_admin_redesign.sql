-- FB-009: submissions admin search, sort, and batch delete support
...
CREATE OR REPLACE FUNCTION public.delete_form_submissions(
  p_tenant_id UUID,
  p_form_id UUID,
...
BEGIN
  ...
  WITH deleted AS (
    ...
    WHERE fs.tenant_id = p_tenant_id
      AND fs.form_id = p_form_id
      AND fs.id = ANY(p_submission_ids)
    RETURNING fs.id
  )
  SELECT jsonb_build_object(
    'deleted_count', COUNT(*)::INTEGER,
    'deleted_ids', COALESCE(jsonb_agg(id), '[]'::jsonb)
  )
  INTO v_result
  FROM deleted;
...
GRANT EXECUTE ON FUNCTION public.delete_form_submissions(UUID, UUID, UUID[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.delete_form_submissions(UUID, UUID, UUID[]) TO service_role;