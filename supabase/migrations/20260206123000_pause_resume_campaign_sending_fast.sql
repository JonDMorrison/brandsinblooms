-- Make pause/resume RPCs finish quickly even on very large campaigns.
-- PostgREST/Supabase often enforces short statement timeouts; avoid updating the entire campaign in one request.
-- We update only a bounded number of rows per call and never wait on locked rows.
...
BEGIN
  ...
  -- This keeps the RPC responsive even for 20k+ campaigns.
  WITH to_pause AS (
    SELECT m.ctid
...
BEGIN
  ...
  WITH to_resume AS (
    ...
    WHERE m.campaign_id = p_campaign_id
      AND m.status = 'paused'
      AND m.resend_id IS NULL
    ORDER BY m.updated_at NULLS LAST
    LIMIT v_message_limit
    FOR UPDATE SKIP LOCKED
  ), u AS (
    UPDATE public.email_messages m
