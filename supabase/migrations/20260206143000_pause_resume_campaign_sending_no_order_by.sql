-- Remove ORDER BY from pause/resume bounded updates.
-- The previous ORDER BY could force sorting large campaigns before LIMIT,
-- which can still hit Supabase/PostgREST statement timeouts.
--
-- Strategy: keep FOR UPDATE SKIP LOCKED + LIMIT, but avoid an expensive sort so
-- the planner can use idx_email_messages_campaign_status and stop early.
...
BEGIN
  ...
  -- NOTE: No ORDER BY here; ORDER BY can force sorting the entire campaign before LIMIT.
  WITH to_pause AS (
    SELECT m.ctid
