-- Fix event-name mismatch in customer analytics RPCs.
--
-- These functions filtered email_tracking_events on event_type IN
-- ('open','click'), but the webhook writer has always inserted
-- 'opened'/'clicked' — so every one of them silently returned empty
-- for webhook-ingested events. Re-created with both spellings.
-- ('click' rows also exist historically from the first-party
-- redirect-click writer, which now writes 'clicked'.)
...
BEGIN
  IF p_channel = 'email' THEN
    ...
    WHERE ete.customer_id = p_customer_id
      AND ete.event_type IN ('open', 'opened', 'click', 'clicked')
...
BEGIN
  ...
  WITH all_events AS (
    ...
    SELECT 
      ...
      CASE 
        WHEN e.event_type IN ('open', 'opened', 'click', 'clicked') THEN 'positive'
...
END;
$$;
-- crm_email_sends has no writer anywhere in the codebase and is empty
-- for every recent campaign. Document that so future code doesn't
-- read from it. (Last reader removed from CRMCampaignReport.tsx.)
COMMENT ON TABLE public.crm_email_sends IS
  'DEPRECATED: no writer since the email_messages ledger replaced it. Do not read from this table; use email_messages / email_tracking_events.';
