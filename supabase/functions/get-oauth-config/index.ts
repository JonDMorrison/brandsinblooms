...
serve(async (req) => {
  ...
  try {
    ...
    if (!finalClientId) {
      console.error(
        `❌ FB_CLIENT_ID not configured for ${environment} environment`,
      );
      return new Response(
        JSON.stringify({
          success: false,
    ...
    console.log(
      `📋 OAuth config response: env=${environment}, clientId=${finalClientId.substring(0, 6)}...`,
    );
    console.log(
      `✅ Providing OAuth config for ${environment} - Client ID source: ${credentialResolution.clientIdSource || "missing"}`,
    );
