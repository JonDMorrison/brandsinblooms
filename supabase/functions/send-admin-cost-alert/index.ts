...
serve(async (req) => {
  ...
  try {
    ...
    const totalEmailsToday =
      emailCampaigns?.reduce((sum, c) => sum + (c.total_sent || 0), 0) || 0;
    ...
    if (totalEmailsToday > THRESHOLDS.dailyEmails) {
      alerts.push({
        ...
        severity:
          totalEmailsToday > THRESHOLDS.dailyEmails * 2
            ? "critical"
            : "warning",
    ...
    const totalSmsToday =
      smsCampaigns?.reduce((sum, c) => sum + (c.total_enqueued || 0), 0) || 0;
    ...
    if (totalSmsToday > THRESHOLDS.dailySms) {
      alerts.push({
        ...
        severity:
          totalSmsToday > THRESHOLDS.dailySms * 2 ? "critical" : "warning",
    ...
    const { data: budgets } = await supabase
      ...
      .select("tenant_id, plan, max_email_sends, max_sms_sends, max_sync_jobs")
      .eq("month", monthKey);

    const { data: counters } = await supabase
      .from("org_usage_counters")
      .select(
        "tenant_id, email_sends_used, sms_sends_used, sync_jobs_used, month",
      )
      .eq("month", monthKey);
    ...
    if (budgets && counters) {
      ...
      const tenantIds = Array.from(
        new Set(counters.map((counter) => counter.tenant_id)),
      );
      const { data: tenants } =
        tenantIds.length === 0
          ? { data: [] }
      ...
      for (const counter of counters) {
        ...
        const tenantName =
          tenantNameMap.get(counter.tenant_id) || counter.tenant_id;
        const emailPercent =
          budget.max_email_sends > 0
            ? (counter.email_sends_used / budget.max_email_sends) * 100
            : 0;
        const smsPercent =
          budget.max_sms_sends > 0
            ? (counter.sms_sends_used / budget.max_sms_sends) * 100
            : 0;
        const syncPercent =
          budget.max_sync_jobs > 0
            ? (counter.sync_jobs_used / budget.max_sync_jobs) * 100
            : 0;

        if (emailPercent >= THRESHOLDS.tenantBudgetPercent) {
          alerts.push({
            type: "Tenant Budget Alert",
            message: `${tenantName} is at ${emailPercent.toFixed(0)}% of email budget`,
            severity: emailPercent >= 100 ? "critical" : "warning",
            details: {
              tenantId: counter.tenant_id,
              plan: budget.plan,
              emailSendsUsed: counter.email_sends_used,
              maxEmailSends: budget.max_email_sends,
            },
          });
        }
        ...
        if (smsPercent >= THRESHOLDS.tenantBudgetPercent) {
          alerts.push({
            type: "Tenant Budget Alert",
            message: `${tenantName} is at ${smsPercent.toFixed(0)}% of SMS budget`,
            severity: smsPercent >= 100 ? "critical" : "warning",
            details: {
              tenantId: counter.tenant_id,
              plan: budget.plan,
              smsSendsUsed: counter.sms_sends_used,
              maxSmsSends: budget.max_sms_sends,
            },
          });
        }

        if (syncPercent >= THRESHOLDS.tenantBudgetPercent) {
          alerts.push({
            type: "Tenant Sync Budget Alert",
            message: `${tenantName} is at ${syncPercent.toFixed(0)}% of sync job budget`,
            severity: syncPercent >= 100 ? "critical" : "warning",
            details: {
              tenantId: counter.tenant_id,
              plan: budget.plan,
              syncJobsUsed: counter.sync_jobs_used,
              maxSyncJobs: budget.max_sync_jobs,
            },
          });
        }
      }
    ...
    if (alerts.length > 0) {
      ...
      for (const adminEmail of SUPER_ADMIN_EMAILS) {
        try {
          ...
          logStep("Failed to send alert email", {
            email: adminEmail,
