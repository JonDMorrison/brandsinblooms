...
async function fetchGoogleAnalyticsData(
  ...
  const overview = {
    ...
    bounceRate:
      parseFloat(overviewData.rows?.[0]?.metricValues?.[4]?.value || "0") * 100,
    engagementRate:
      parseFloat(overviewData.rows?.[0]?.metricValues?.[5]?.value || "0") * 100,
  };
  ...
  const topCountries =
    countriesData.rows?.map((row: any) => ({
      country: row.dimensionValues[0].value,
      sessions: parseInt(row.metricValues[0].value),
    })) || [];

  const deviceBreakdown =
    devicesData.rows?.map((row: any) => ({
  ...
  try {
    ...
    const mockData = {
      overview: {
        ...
        avgSessionDuration: Math.floor(Math.random() * 240) + 45,
        bounceRate: Math.random() * 35 + 20,
        engagementRate: Math.random() * 40 + 30,
      },
      dailyData: Array.from({ length: dateRange }, (_, i) => {
        ...
        return {
          ...
          pageviews: Math.floor(Math.random() * 700) + 120,
        };
      }),
      topCountries: [
        {
