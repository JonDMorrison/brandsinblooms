...
async function getAdapter(platform: string, credentials: any) {
  switch (platform.toLowerCase()) {
    case "mock":
      const { MockAdapter } =
        await import("../../../src/components/crm/pos/adapters/MockAdapter.ts");
      return new MockAdapter();
    case "vmx":
      const { VMXAdapter } =
        await import("../../../src/components/crm/pos/adapters/VMXAdapter.ts");
      return new VMXAdapter();
    case "shopify":
      // SHOPIFY BRANCH DEPRECATED
      // This branch is superseded by syncShopifyCustomers/Orders/Products in pos-sync-worker.
      // See shopify-full-sync for the new entry point.
      // TODO: Remove this branch after new integration soak period ends.
      const { ShopifyAdapter } =
        await import("../../../src/components/crm/pos/adapters/ShopifyAdapter.ts");
    ...
    case "square":
      const { SquareAdapter } =
        await import("../../../src/components/crm/pos/adapters/SquareAdapter.ts");
      return new SquareAdapter(
        credentials.access_token,
        credentials.environment || "production",
      );
    case "counterpoint":
      const { CounterpointAdapter } =
        await import("../../../src/components/crm/pos/adapters/CounterpointAdapter.ts");
      return new CounterpointAdapter(
        credentials.api_key,
  ...
  try {
    ...
    while (hasMore) {
      try {
        ...
        if (totalFetched >= page_limit * 10) {
          // Max 10 pages per sync
          break;
        }
      } catch (syncError) {
    ...
    await supabase.from("pos_sync_logs").update(syncResult).eq("id", logId);
