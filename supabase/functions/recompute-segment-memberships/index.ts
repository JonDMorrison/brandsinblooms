...
serve(async (req) => {
  ...
  try {
    const supabase = createClient(
      ...
      { db: { schema: "public" }, global: { headers: { "x-supabase-max-rows": "50000" } } },
    );
    ...
    if (!segments || segments.length === 0) {
      return new Response(
        JSON.stringify({ segments_evaluated: 0, customers_evaluated: 0, memberships_added: 0, memberships_removed: 0, duration_ms: Date.now() - startTime }),
    ...
    const selectFields = "id, email, first_name, last_name, total_spent, last_visit_date, last_purchase_date, loyalty_member, loyalty_rewards_balance, email_consent, sms_consent, tags, pos_source, custom_fields, created_at";
    ...
    if (customer_ids?.length > 0) {
      ...
      while (true) {
        ...
        if (pgErr) throw new Error(`Failed to load customers page ${offset}: ${pgErr.message}`);
    ...
    if (customers.length === 0) {
      return new Response(
        JSON.stringify({ segments_evaluated: segments.length, customers_evaluated: 0, memberships_added: 0, memberships_removed: 0, duration_ms: Date.now() - startTime }),
        { status: 200, headers: { "Content-Type": "application/json" } },
      );
    }
    ...
    // 4. Evaluate and diff
    const toInsert: Array<{ customer_id: string; segment_id: string; assigned_at: string }> = [];
    ...
    for (const customer of customers) {
      for (const segment of segments) {
        ...
        const matches = evaluateConditions(conditions, customer);
        const exists = existingSet.has(key);

        if (matches && !exists) {
          toInsert.push({ customer_id: customer.id, segment_id: segment.id, assigned_at: new Date().toISOString() });
    ...
    console.log(`recompute-segment-memberships: ${segments.length} segments, ${customers.length} customers, +${toInsert.length}/-${toDelete.length}, ${duration}ms`);
