  ...
  try {
    ...
    for (const listId of listIds) {
      ...
      while (pageUrl) {
        ...
        if (profilesRes.status === 401) {
          throw new Error(
            "Access token expired. Please reconnect Klaviyo and try again.",
          );
        }
          ...
          try {
            ...
            const consentStatus =
              attrs.subscriptions?.email?.marketing?.consent === "SUBSCRIBED"
                ? "opted_in"
                : attrs.subscriptions?.email?.marketing?.consent ===
                    "UNSUBSCRIBED"
                  ? "opted_out"
                  : "suppressed";
            ...
            if (
              ...
              const suppressionType =
                consentStatus === "opted_out" ? "unsubscribed" : "unsubscribed";
    ...
    await supabase
      ...
      .update({
        ...
        report: {
          ...
          contacts_skipped: 0,
          contacts_failed: totalErrors,
          segments_created: 0,
          tags_created: 0,
          consents_recorded: totalContacts,
          errors: detailedErrors.map(
            (entry) => `${entry.item}: ${entry.error}`,
          ),
          batches_processed: Math.max(1, Math.ceil(totalContacts / batchSize)),
        },
      })
      .eq("id", importJob.id);

    console.log(
      `[klaviyo-import] Completed job ${importJob.id}: ${totalContacts} contacts`,
    );
    ...
    return new Response(
      JSON.stringify({
        ...
        segmentsImported: 0,
        errors: detailedErrors.slice(0, 10),
        progress: {
          stage: "complete",
          contactsProcessed: totalContacts,
