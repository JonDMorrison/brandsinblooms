 ...
 * 
 * Background health check for Clover webhook subscriptions.
...
Deno.serve(async (req) => {
  ...
  console.log('[CLOVER-WEBHOOK-HEALTH] Starting health check');
  ...
  try {
    ...
    if (error) {
      console.error('[CLOVER-WEBHOOK-HEALTH] Failed to fetch connections:', error.message);
      throw error;
    }

    console.log('[CLOVER-WEBHOOK-HEALTH] Found', connections?.length || 0, 'connections needing attention');

    const results: any[] = [];
    const MAX_RETRIES = WEBHOOK_RETRY_CONFIG.MAX_RETRIES;

    for (const conn of connections || []) {
      // Skip if not due for retry yet
      if (conn.webhook_next_retry_at && new Date(conn.webhook_next_retry_at) > new Date()) {
        console.log(`[CLOVER-WEBHOOK-HEALTH] Skipping ${conn.id} - retry not due until ${conn.webhook_next_retry_at}`);
        results.push({
      ...
      if ((conn.webhook_retry_count || 0) >= MAX_RETRIES) {
        console.log(`[CLOVER-WEBHOOK-HEALTH] Skipping ${conn.id} - max retries (${MAX_RETRIES}) exceeded`);
        results.push({
          connection_id: conn.id,
          merchant_id: conn.merchant_id,
          action: 'skipped',
      ...
      console.log(`[CLOVER-WEBHOOK-HEALTH] Retrying webhook setup for ${conn.id} (attempt ${(conn.webhook_retry_count || 0) + 1})`);

      try {
        const result = await ensureCloverWebhooks(supabase, conn.id);
        results.push({
          connection_id: conn.id,
          merchant_id: conn.merchant_id,
        ...
        if (result.verified) {
          console.log(`[CLOVER-WEBHOOK-HEALTH] ✓ ${conn.id} webhook verified`);
        } else {
          console.log(`[CLOVER-WEBHOOK-HEALTH] ⚠ ${conn.id} webhook pending: ${result.error}`);
        }
      } catch (e: any) {
        console.error(`[CLOVER-WEBHOOK-HEALTH] Error processing ${conn.id}:`, e.message);
        results.push({
          connection_id: conn.id,
          merchant_id: conn.merchant_id,
          action: 'error',
    ...
    if (staleConnections?.length) {
      console.log(`[CLOVER-WEBHOOK-HEALTH] Found ${staleConnections.length} stale connections to re-verify`);
      ...
      for (const conn of staleConnections) {
        try {
          ...
          console.error(`[CLOVER-WEBHOOK-HEALTH] Stale check error for ${conn.id}:`, e.message);
        }
      }
    }
    ...
    const summary = {
      ...
      pending: results.filter(r => r.action === 'updated').length,
    };

    console.log('[CLOVER-WEBHOOK-HEALTH] Complete:', JSON.stringify(summary));
    ...
    console.error('[CLOVER-WEBHOOK-HEALTH] Error:', error.message);
    return new Response(
