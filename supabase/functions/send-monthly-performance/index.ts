 ...
 * Sends each Active client a personalised monthly performance summary email.
 */
...
function trendBadge(current: number, prev: number): string {
  ...
  if (pct > 0)
    return `<span style="font-size:12px;color:#1abc9c;font-weight:500;">↑ ${pct}%</span>`;
  if (pct < 0)
    return `<span style="font-size:12px;color:#ef4444;font-weight:500;">↓ ${Math.abs(pct)}%</span>`;
  return `<span style="font-size:12px;color:#6b7280;">→ flat</span>`;
}

function getProvinceContext(province: string): string {
  if (province === "BC")
    return "BC garden centers have one of the longest seasons in Canada — spring opens in March/April and extends into November.";
  if (["AB", "SK", "MB"].includes(province))
    return "Prairie garden centers have a short, intense season. Spring opens in May and every week of peak season counts.";
  if (["ON", "QC"].includes(province))
    return "Central Canadian garden centers see spring open in late April to May with strong spring and fall selling windows.";
  if (["NS", "NB", "NL"].includes(province))
    return "Atlantic Canadian garden centers have a short window — spring opens in May and the season moves fast.";
...
Deno.serve(async () => {
  ...
  try {
    // ── STEP 1: Get Active clients from Notion ──
    ...
    // Seasonal hero image and accent color
    ...
    for (const page of pages) {
      ...
      if (!email || !tenantId) {
        ...
        results.push({ email: email || gardenCenter, status: "skipped_missing_data" });
        continue;
      }
      ...
      try {
        ...
        const [
          ...
          supabase.from("crm_campaigns").select("id", { count: "exact", head: true })
            .eq("tenant_id", tenantId).eq("status", "sent")
            .gte("sent_at", firstOfLastMonth.toISOString()).lt("sent_at", firstOfThisMonth.toISOString()),
          supabase.from("crm_campaigns").select("id", { count: "exact", head: true })
            .eq("tenant_id", tenantId).eq("status", "sent")
            .gte("sent_at", firstOfTwoMonthsAgo.toISOString()).lt("sent_at", firstOfLastMonth.toISOString()),
          supabase.from("crm_customers").select("id", { count: "exact", head: true })
            .eq("tenant_id", tenantId),
          supabase.from("crm_customers").select("id", { count: "exact", head: true })
            .eq("tenant_id", tenantId)
            .gte("created_at", firstOfLastMonth.toISOString()).lt("created_at", firstOfThisMonth.toISOString()),
          supabase.from("crm_customers").select("id", { count: "exact", head: true })
            .eq("tenant_id", tenantId)
            .gte("created_at", firstOfTwoMonthsAgo.toISOString()).lt("created_at", firstOfLastMonth.toISOString()),
          supabase.from("crm_automations").select("id", { count: "exact", head: true })
            .eq("tenant_id", tenantId).eq("is_active", true),
        ]);

        const campaignCount = campaignsLastMonth.count ?? 0;
        const campaignCountPrev = campaignsPrevMonth.count ?? 0;
        const totalContacts = totalContactsResult.count ?? 0;
        const newContacts = newContactsLastMonth.count ?? 0;
        const newContactsPrev = newContactsPrevMonth.count ?? 0;
        const automationCount = automationsResult.count ?? 0;
        ...
        try {
          ...
          if (userRow?.id) {
            const { data: profile } = await supabase.from("company_profiles")
              .select("company_overview, usda_zone, postal_code")
          ...
          const aiRes = await fetch("https://api.openai.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
              ...
              messages: [
                {
                  role: "system",
                  content: "You are a marketing coach for independent garden centers. Write exactly ONE encouraging, specific sentence under 25 words commenting on this garden center's month. Use their stats, location, and specializations. Sound like a knowledgeable human advisor, not a bot.",
                },
                {
                  role: "user",
                  content: `Garden center: ${gardenCenter}. ${locationContext} What they sell: ${companyOverview || "general garden center"}. Month: ${monthName}. Campaigns sent: ${campaignCount} (${campaignCountPrev > 0 ? (campaignCount >= campaignCountPrev ? "+" : "") + Math.round(((campaignCount - campaignCountPrev) / Math.max(campaignCountPrev, 1)) * 100) + "% vs last month" : "first month tracked"}). Total contacts: ${totalContacts}. New contacts this month: ${newContacts}. Active automations: ${automationCount}.`,
                },
              ],
            }),
          });
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
      <tr>
        <td style="width:50%;padding:0 6px 12px 0;vertical-align:top;">
          <div style="background:#f9fafb;border-radius:10px;border:1px solid #e5e7eb;padding:16px;text-align:center;">
            <p style="font-size:32px;font-weight:600;color:#0d1f1a;margin:0 0 2px;line-height:1;">${campaignCount}</p>
            <p style="font-size:11px;color:#6b7280;margin:0 0 6px;text-transform:uppercase;letter-spacing:0.5px;">Campaigns sent</p>
            ${trendBadge(campaignCount, campaignCountPrev)}
          </div>
        </td>
      ...
      <tr>
        <td style="width:50%;padding:0 6px 0 0;vertical-align:top;">
          <div style="background:#f9fafb;border-radius:10px;border:1px solid #e5e7eb;padding:16px;text-align:center;">
            ...
            <p style="font-size:11px;color:#6b7280;margin:0 0 6px;text-transform:uppercase;letter-spacing:0.5px;">New contacts</p>
            ${trendBadge(newContacts, newContactsPrev)}
          </div>
        </td>
    ...
    <p style="font-size:12px;color:#9ca3af;margin:0;">You received this because you are a BloomSuite member. <a href="https://www.bloomsuite.app/unsubscribe" style="color:#9ca3af;">Unsubscribe</a></p>
        ...
        const resendRes = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            to: email,
            subject: `${gardenCenter} — your ${monthName} marketing summary`,
            html: emailHtml,
          }),
        });
        ...
        if (!resendRes.ok) {
          ...
          console.error(`send-monthly-performance: Resend failed for ${email}`, resendRes.status, body);
          results.push({ email, status: "resend_failed" });
        } else {
          console.log(`send-monthly-performance: sent to ${email}`);
          results.push({ email, status: "sent" });
        }
      } catch (clientErr) {
        console.error(`send-monthly-performance: error processing ${email}`, clientErr);
        results.push({ email: email || gardenCenter, status: "error" });
      }
    }

    const sent = results.filter((r) => r.status === "sent").length;
    console.log(`send-monthly-performance: ${sent}/${results.length} emails sent`);
