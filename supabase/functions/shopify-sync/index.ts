/**
 * @deprecated
 * This function is superseded by the new first-class Shopify integration pipeline.
 * Use shopify-full-sync -> pos-sync-worker (Shopify branch) instead.
 * This file is retained for reference and will be deleted after the 30-day soak period
 * following the launch of the new integration.
 *
...
class ShopifyAdapter {
  ...
  async fetchCustomers(): Promise<any> {
    const response = await fetch(
      `${this.baseUrl}/admin/api/2024-01/customers.json`,
      {
        headers: {
  ...
  async fetchOrders(): Promise<any> {
    const response = await fetch(
      `${this.baseUrl}/admin/api/2024-01/orders.json?status=any&limit=250`,
      {
        headers: {
          "X-Shopify-Access-Token": this.accessToken,
          "Content-Type": "application/json",
  ...
  adaptCustomers(rawCustomers: any[]): NormalizedCustomer[] {
    return rawCustomers.map((customer) => ({
      ...
      address: customer.default_address
        ? {
            street:
              `${customer.default_address.address1 || ""} ${customer.default_address.address2 || ""}`.trim(),
