  ...
  try {
    ...
    for (const listId of listIds) {
      ...
      while (hasMore) {
        ...
        if (!contactsRes.ok) {
          ...
          console.error(
            `[constant-contact-import] Error fetching list ${listId}:`,
            contactsRes.status,
            errorText,
          );
        ...
        for (let i = 0; i < contacts.length; i += batchSize) {
          ...
          for (const contact of batch) {
            ...
            if (existingCustomer) {
              ...
              if (updateError) {
                console.error(
                  "[constant-contact-import] Update error:",
                  updateError,
                );
              }
              ...
              if (insertError) {
                console.error(
                  "[constant-contact-import] Insert error:",
                  insertError,
                );
                errors.push(
                  `Failed to import ${email}: ${insertError.message}`,
                );
                continue;
              }
        ...
        console.log(
          `[constant-contact-import] Processed ${contacts.length} contacts from list ${listId}, cursor: ${cursor ? "has more" : "done"}`,
        );
      }
    }
    ...
    if (processed > 0) {
      try {
        ...
        console.warn(
          "[constant-contact-import] Failed to record import activity event:",
          importEventError?.message || importEventError,
        );
      }
    }
      ...
      await supabase
        ...
        .update({
          ...
          report: {
            ...
            contacts_failed: errors.length,
            segments_created: 0,
            tags_created: 0,
            consents_recorded: imported,
            errors: errors.slice(0, 100),
            batches_processed: Math.max(1, Math.ceil(processed / 50)),
          },
    ...
    console.log(
      `[constant-contact-import] Import complete: ${imported} imported, ${skipped} skipped, ${errors.length} errors`,
    );
