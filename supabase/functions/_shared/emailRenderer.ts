...
import { generateServerFooterHtml, type CompanyProfileData } from "./footerGenerator.ts";
 ...
 */
function findEmptyResolvedTags(content: string, mergeData: MergeTagData): string[] {
 ...
 */
export function renderEmailForRecipient(params: RenderEmailParams): RenderEmailResult {
  const { 
    tenantId,
    campaignId,
    subject = '',
  ...
  console.log(`[emailRenderer] mode=${mode}, tenantId=${tenantId}, customer=${customer?.email || 'sample'}`);
  
  // Step 1: Normalize escaped merge tokens
  let normalizedHtml = normalizeMergeTokens(html);
  let normalizedSubject = normalizeMergeTokens(subject);
  
  // Step 2: Count legacy tags before conversion
  const legacyTagsConverted = countLegacyTags(normalizedHtml) + countLegacyTags(normalizedSubject);
  ...
  const emptyResolvedTags = findEmptyResolvedTags(normalizedHtml + normalizedSubject, mergeData);
  ...
  if (includeFooter && mode === 'send' && customer) {
    renderedHtml = appendFooter(renderedHtml, customer, companyProfile, tenantId);
  }
  
  // Step 9: Log diagnostics
  console.log(`[emailRenderer] usedTags=${usedTags.length}, legacy=${legacyTagsConverted}, missing=${missingTags.length}, empty=${emptyResolvedTags.length}`);
...
function buildMergeData(
  ...
  const baseUnsubscribeUrl = customer?.email 
    ...
    : '#';
  const basePreferencesUrl = baseUnsubscribeUrl.replace('handle-unsubscribe', 'manage-preferences');
  ...
  if (customer) {
    const mergeData = createMergeTagDataFromCustomer(
      customer as Record<string, unknown>,
      {
        ...
        address: companyProfile?.location_info || companyProfile?.street_address || undefined,
...
function appendFooter(
  ...
  let strippedHtml = stripExistingFooter(html);
