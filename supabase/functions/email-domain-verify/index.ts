...
async function verifyDNSRecordDirectly(
  ...
  for (const value of result.values) {
    if (type === "MX") {
      ...
      const mxHost =
        (parts.length > 1 ? parts[1] : parts[0])?.replace(/\.$/, "") || "";

      if (
        priority !== undefined &&
        mxPriority === priority &&
        mxHost === expectedValue
      ) {
        found = true;
        break;
      }
  ...
  console.log(
    JSON.stringify({
      ...
      type,
      expectedValue:
        expectedValue.substring(0, 50) +
        (expectedValue.length > 50 ? "..." : ""),
      found,
 ...
 * - DMARC exists with p=none or stronger
 *
 ...
 * 3. ACTION_REQUIRED_AUTHENTICATION: DNS exists but provider/DMARC is incomplete
 * 4. CONNECTED_READY: DNS and authentication are verified
...
function computeReadinessStatus(params: {
  ...
  // Priority 3: required DNS is present, but authentication is not complete.
  if (allDnsVerified && (!allProviderVerified || !dmarcVerified)) {
    const missing = [
      !allProviderVerified ? "provider verification" : null,
      !dmarcVerified ? "DMARC" : null,
    ]
      .filter(Boolean)
      .join(" and ");
    return {
      status: "ACTION_REQUIRED_AUTHENTICATION",
      message: `Email authentication is incomplete: ${missing}.`,
      subMessage:
  ...
  // Priority 4: DNS and provider authentication are fully verified.
  if (allDnsVerified && allProviderVerified && dmarcVerified) {
    return {
...
function calculateNextVerifyAt(
  attempts: number,
  allPassed: boolean,
): Date | null {
...
function mapResendStatus(
  ...
  allDnsVerifiedIndependently: boolean,
): string {
  if (
    allChecksPassed &&
    allDnsVerifiedIndependently &&
    resendStatus === "verified"
  )
    return "active";
  if (allDnsVerifiedIndependently) return "verifying";
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { domainId, email_domain_id, reset_attempts }: VerifyDomainRequest =
      await req.json();
    ...
    console.log(
      `🔍 Verifying domain: ${finalDomainId}, reset_attempts: ${reset_attempts}`,
    );
    ...
    if (!reset_attempts && lastAttempt) {
      ...
      if (timeSinceLastAttempt < minIntervalMs) {
        const retryAfterSeconds = Math.ceil(
          (minIntervalMs - timeSinceLastAttempt) / 1000,
        );
        console.log(
          `⏳ Rate limited: ${retryAfterSeconds}s until next attempt allowed`,
        );
        return corsJsonResponse(
          {
            error: "rate_limited",
    ...
    if (currentAttempts >= 10 && !reset_attempts) {
      ...
      return corsJsonResponse({
        ...
        status: "failed",
        message:
          'Maximum verification attempts (10) reached. Click "Retry" to reset and try again.',
    ...
    if (!emailDomain.resend_domain_id) {
      return corsJsonResponse(
        {
          error:
            "No Resend domain ID found. Domain may not be properly provisioned.",
    ...
    const recordDnsStatus: Record<
      string,
      DnsVerifyResult & { hasConflict?: boolean }
    > = {};
    ...
    try {
      console.log(
        `📧 Triggering Resend verification for: ${emailDomain.resend_domain_id}`,
      );

      // Step 0: Ensure open and click tracking is enabled for this domain
      try {
        console.log(
          `📊 Enabling open and click tracking for domain ${emailDomain.resend_domain_id}...`,
        );
        ...
        if (updateError) {
          console.warn(
            `⚠️ Failed to enable tracking (non-fatal):`,
            updateError,
          );
        } else {
      ...
      try {
        ...
        if (verifyError) {
          console.log(
            `⚠️ Resend verify returned error:`,
      ...
      const { data: domainStatus, error: statusError } =
        await resend.domains.get(emailDomain.resend_domain_id);
        ...
        return corsJsonResponse(
          {
            ...
            message:
              "Unable to check domain status with Resend. Please try again.",
            details: statusError,
          },
          { status: 400 },
        );
      ...
      console.log(
        ...
        JSON.stringify(domainStatus, null, 2),
      );
      ...
      const spfCheck: DNSCheck = {
        ...
        details: {
          ...
          records: resendRecords.filter(
            (r: ResendRecord) => r.record_type === "TXT" || r.type === "TXT",
          ),
        },
      };
      ...
      const verificationCheck: DNSCheck = {
        ...
        details: {
          ...
          verified_count: resendRecords.filter(
            (r: ResendRecord) => r.status === "verified",
          ).length,
        },
      };
      ...
      console.log(
        `📊 Resend verification results: DKIM=${dkimVerified}, SPF=${spfVerified}, ReturnPath=${returnPathVerified}, Overall=${domainStatus.status}`,
      );
      ...
      for (const record of resendRecords) {
        ...
        console.log(
          JSON.stringify({
            ...
            fqdnQueried: fqdn,
            expectedValue:
              recordValue.substring(0, 50) +
              (recordValue.length > 50 ? "..." : ""),
            resolver: DNS_RESOLVER,
          }),
        );
        ...
        if (
          ...
          !recordName.includes("_domainkey")
        ) {
        ...
        if (recordType === "TXT") {
          verifyResult = await verifyDNSRecordDirectly(
            fqdn,
            "TXT",
            recordValue,
          );
      ...
      // =========================================================
      console.log(
        `🔍 Checking for DNS conflicts at ${hostnamesForConflictCheck.size} hostname(s)...`,
      );
        ...
        if (conflict.hasConflict) {
          console.log(
            JSON.stringify({
              ...
              message:
                "CNAME conflicts with MX/TXT records - invalid DNS configuration",
            }),
          );
      ...
      console.log(
        `📊 Independent DNS results: DKIM=${dkimDnsVerified}, SPF=${spfDnsVerified}, MX=${mxDnsVerified}, AllDNS=${allDnsVerified}, ConflictDetected=${dnsConflictDetected}`,
      );
      ...
      const returnPathCheckRef = checks.find(
        (c) => c.check_name === "return_path",
      );
    ...
    for (const check of checks) {
      ...
      if (checkError) {
        console.error(
          `❌ Failed to save check ${check.check_name}:`,
          checkError,
        );
      }
    }
    ...
    const dnsVerifiedButResendPending =
      allDnsVerified && !allPassed && !dnsConflictDetected;
    ...
    const errorMessage = treatedAsActive
      ? null
      : allPassed
        ? null
        : buildVerificationError(checks, resendRecords, dnsConflictDetected);
    const nextVerifyAt = treatedAsActive
      ? null
      : calculateNextVerifyAt(newAttempts, allPassed);

    console.log(
      `📊 Status update: ${emailDomain.status} -> ${newStatus}, phase: ${verificationPhase}, attempts: ${newAttempts}, next_verify: ${nextVerifyAt?.toISOString() || "none"}`,
    );
    ...
    const updateData: any = {
      ...
      resend_status: {
        ...
        return_path_verified:
          checks.find((c) => c.check_name === "return_path")?.ok || false,
        dmarc_verified:
          checks.find((c) => c.check_name === "dmarc")?.ok || false,
    ...
    if (newStatus === "active") {
      ...
      if (tenantUser?.id) {
        ...
        if (profileError) {
          ...
          console.log(
            `✅ Synced domain ${emailDomain.domain} to company_profile for user ${tenantUser.id}`,
          );
        }
      }
    }
    ...
    console.log(
      `🎉 Domain verification completed: ${emailDomain.domain} -> ${newStatus} (phase: ${verificationPhase})`,
    );
    ...
    const actualCta =
      readinessResult.cta && entriRepairAvailable ? readinessResult.cta : null;

    const response = {
      ok: allPassed && dmarcOk,
      allVerified: allPassed && dmarcOk,
      dns_verified: allDnsVerified,
