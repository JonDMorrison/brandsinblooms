  ...
  try {
    ...
    if (media_url) {
      ...
      const mediaResponse = await fetch(
        `https://graph.facebook.com/${GRAPH_API_VERSION}/${connection.platform_account_id}/media`,
        {
          method: "POST",
      ...
      if (!mediaResponse.ok || !mediaResult.id) {
        throw new Error(
          mediaResult.error?.message || "Failed to create Instagram media",
        );
      }

      // Publish the media
      const publishResponse = await fetch(
        `https://graph.facebook.com/${GRAPH_API_VERSION}/${connection.platform_account_id}/media_publish`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!publishResponse.ok) {
        throw new Error(
          result.error?.message || "Failed to publish Instagram post",
        );
      }
    } else {
      // Text-only post (Story or Reel - Instagram doesn't support text-only feed posts)
      throw new Error(
        "Instagram requires media for feed posts. Please add an image.",
      );
    }
    ...
    if (result.id) {
      ...
      const errorMessage =
        result.error?.message || "Failed to post to Instagram";
