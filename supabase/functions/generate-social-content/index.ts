...
serve(async (req) => {
  ...
  const _supabaseAuth = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_ANON_KEY')!, { global: { headers: { Authorization: authHeader } } });
  const { error: authError } = await _supabaseAuth.auth.getUser();
  if (authError) {
    return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }
...
Return valid JSON only.`;
    ...
    // ✅ Images are generated by frontend AI system (Lovable AI)
    // This edge function only generates text content
    console.log('[generate-social-content] ✅ Text content generated, images will be created by frontend');
    console.log('[generate-social-content] 🤖 No Unsplash fetching - AI images via frontend with centralized storage');
    ...
    return new Response(
      JSON.stringify({
        ...
        // ✅ No images property - frontend will generate AI images
      }),
