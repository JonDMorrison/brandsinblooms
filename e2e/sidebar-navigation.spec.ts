...
test.describe("Dashboard shell navigation", () => {
  test("integrations route exposes the current shell controls", async ({
    ...
    if (isMobile) {
      ...
      await expect(
        page.locator('[data-testid="dashboard-shell-sidebar-mobile"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="dashboard-shell-backdrop"]'),
      ).toBeVisible();
    ...
    await expect(sidebar.locator('a[href="/integrations"]')).toHaveAttribute(
      "aria-current",
      "page",
    );
  });
