...
test.describe('Admin dashboard shell', () => {
  ...
  test('cost and analytics health pages render inside the admin shell', async ({
    ...
    for (const route of adminRoutes) {
      ...
      await expect(page.locator('[data-testid="dashboard-shell-root"]')).toHaveCount(1);
      await expect(page.locator('[data-testid="dashboard-shell-topbar"]')).toContainText(route.topbarTitle);
