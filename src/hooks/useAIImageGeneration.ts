...
interface UseAIImageGenerationReturn {
  generateSingleImage: (options: AIImageGenerationOptions) => Promise<string | null>;
  generateMultipleImages: (options: AIImageGenerationOptions[]) => Promise<Array<string | null>>;
...
export const useAIImageGeneration = (): UseAIImageGenerationReturn => {
  ...
  const generateSingleImage = useCallback(async (options: AIImageGenerationOptions): Promise<string | null> => {
    if (!user) {
    ...
    try {
      console.log(`🎨 Generating AI image for ${options.channel}:`, options.contentTitle || options.contentContext.substring(0, 50));
  ...
  const generateMultipleImages = useCallback(async (
    ...
    console.log(`🎨 Starting parallel AI image generation for ${optionsArray.length} images`);
    ...
    try {
      const imagePromises = optionsArray.map(async (options, index) => {
        try {
          ...
          console.log(`✅ Image ${index + 1}/${optionsArray.length} generated successfully`);
      ...
      console.log(`🎯 Parallel generation completed: ${results.filter(Boolean).length}/${optionsArray.length} successful`);
