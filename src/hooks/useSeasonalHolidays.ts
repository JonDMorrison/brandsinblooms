...
export const useSeasonalHolidays = () => {
  ...
  const fetchHolidayContentState = useCallback(async () => {
    ...
    try {
      ...
      allHolidays.forEach((holiday) => {
        const holidayTasks =
          tasks?.filter((task) => task.holiday_id === holiday.id) || [];
        const uniquePostTypes = [
          ...new Set(holidayTasks.map((task) => task.post_type)),
        ];
        const hasAllFiveTypes = [
          "instagram",
          "facebook",
          "blog",
          "video",
        ...
        const latestTimestamp =
          holidayTasks.length > 0
            ? Math.max(
                ...holidayTasks.map((t) => new Date(t.created_at).getTime()),
              )
            : undefined;
  ...
  const generateHolidayContentForHoliday = useCallback(
    async (holidayId: string) => {
      ...
      if (!holiday) {
        ...
        console.error(`HOOK ERROR: ${errorMsg}`, {
          holidayId,
          availableHolidays: allHolidays.length,
        });
        throw new Error(errorMsg);
      }
      try {
        const results = await generateHolidayContent(
          user,
          holiday,
          tenant,
        ...
        if (successCount === totalExpected) {
          toast.success(
            `All 5 content pieces generated for ${holiday.holiday_name}!`,
            {
              description:
                "Instagram, Facebook, Blog, Video, and Newsletter are ready for review.",
            },
          );
        } else if (successCount > 0) {
          toast.warning(
            `${successCount}/${totalExpected} content pieces generated`,
            {
        ...
        console.error(
          "HOOK: Error in generateHolidayContentForHoliday:",
          error,
        );
        ...
        if (error?.message) {
          if (error.message.includes("OpenAI")) {
            errorMessage =
              "AI content generation service is currently unavailable";
          } else if (
            ...
            errorMessage =
              "Authentication issue - please refresh and try again";
          } else {
