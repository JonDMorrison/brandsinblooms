...
const defaultFooterSettings: FooterSettings = {
  ...
  fontSize: "xs",
  complianceText:
    "You are receiving this email because you opted in at {{company.name}}. If you no longer wish to receive these emails, you can unsubscribe at any time.",
};
...
export const useFooterSettings = (campaignId?: string) => {
  ...
  const [campaignOverrides, setCampaignOverrides] =
    useState<CampaignFooterOverrides>({});
  ...
  const saveCampaignFooterOverride = useCallback(
    async (
  ...
  const saveFooterStyling = useCallback(
    async (campaignIdToUpdate: string, styling: FooterStyling) => {
      try {
        // Get current campaign metadata
        const { data: campaign, error: fetchError } = await supabase
          .from("crm_campaigns")
        ...
        if (fetchError) {
          console.error(
            "[useFooterSettings] Error fetching campaign:",
            fetchError,
          );
          throw fetchError;
        }
        ...
        const newMetadata = {
          ...
          footerBackgroundColor:
            styling.backgroundColor || currentMetadata.footerBackgroundColor,
        };
  ...
  const clearCampaignFooterOverride = useCallback(
    async (campaignIdToUpdate: string) => {
      try {
        const { data: campaign, error: fetchError } = await supabase
          .from("crm_campaigns")
          .select("metadata")
