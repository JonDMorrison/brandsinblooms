 ...
 */
export const useCustomerEnrollmentHistory = (
  customerId: string | undefined,
) => {
  return useQuery({
...
export const useTenantLoyaltyStats = (tenantId: string | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const tierCounts = {
        bronze: members.filter((m) => m.current_loyalty_tier === "bronze")
          .length,
        silver: members.filter((m) => m.current_loyalty_tier === "silver")
          .length,
        gold: members.filter((m) => m.current_loyalty_tier === "gold").length,
        platinum: members.filter((m) => m.current_loyalty_tier === "platinum")
          .length,
      };

      const stats: TenantLoyaltyStats = {
        perks_enrollment_rate: 0, // Will be calculated via RPC
        avg_time_to_join_days:
          members.length > 0
            ? members.reduce(
                (sum, m) => sum + (m.time_to_join_perks_days || 0),
                0,
              ) / members.length
            : null,
        avg_points_earned:
          members.length > 0
            ? members.reduce((sum, m) => sum + m.total_points_earned, 0) /
              members.length
            : 0,
        avg_points_redeemed:
          members.length > 0
            ? members.reduce((sum, m) => sum + m.total_points_redeemed, 0) /
              members.length
            : 0,
        avg_redemption_frequency:
          members.length > 0
            ? members.reduce((sum, m) => sum + m.redemption_frequency, 0) /
              members.length
            : 0,
        avg_redemption_delay_days:
          members.filter((m) => m.avg_redemption_delay_days).length > 0
            ? members
                .filter((m) => m.avg_redemption_delay_days)
                .reduce(
                  (sum, m) => sum + (m.avg_redemption_delay_days || 0),
                  0,
                ) / members.filter((m) => m.avg_redemption_delay_days).length
            : null,
        total_perks_driven_revenue: members.reduce(
          (sum, m) => sum + m.total_perks_driven_revenue,
          0,
        ),
        avg_non_redeemed_ratio:
          members.length > 0
            ? members.reduce((sum, m) => sum + m.non_redeemed_points_ratio, 0) /
              members.length
            : 0,
        tier_breakdown: tierCounts,
        engagement_difference: {
          member_avg_purchase_frequency:
            members.length > 0
              ? members.reduce(
                  (sum, m) => sum + (m.member_purchase_frequency || 0),
                  0,
                ) / members.length
              : 0,
          non_member_avg_purchase_frequency: 0, // Would need separate query
          member_email_open_rate:
            members.filter((m) => m.member_email_open_rate).length > 0
              ? members
                  .filter((m) => m.member_email_open_rate)
                  .reduce(
                    (sum, m) => sum + (m.member_email_open_rate || 0),
                    0,
                  ) / members.filter((m) => m.member_email_open_rate).length
              : 0,
      ...
      try {
        const { data: enrollmentRate } = await supabase.rpc(
...
export const useRefreshLoyaltyMetrics = () => {
  ...
  return useMutation({
    ...
    onSuccess: (customerId) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["customer-points-history", customerId],
      });
    },
  });
};
 ...
 */
export const refreshAllLoyaltyMetrics = async (
  tenantId: string,
): Promise<number> => {
