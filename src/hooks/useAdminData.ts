...
const PAID_PLAN_PRICES: Record<string, number> = {
  seed: 199,
  sprout: 349,
  bloom: 699,
  thrive: 1199,
...
export const useAdminData = () => {
  ...
  const fetchAdminData = async () => {
    try {
      ...
      const activePaidAccounts =
        adminData?.filter(
          (user) =>
            user.subscription_plan in PAID_PLAN_PRICES &&
            user.subscription_status === "active",
        ).length || 0;
      ...
      adminData?.forEach((user) => {
        if (
          ...
          currentMRR +=
            PAID_PLAN_PRICES[
              user.subscription_plan as keyof typeof PAID_PLAN_PRICES
            ];
        }
      });

      // Calculate potential MRR if all trial accounts converted to Seed.
      const potentialMRR = activeTrialAccounts * PAID_PLAN_PRICES.seed;

      // Calculate conversion rate based on unique users
      const conversionRate =
        uniqueUsers > 0
          ? Math.round((activePaidAccounts / uniqueUsers) * 100)
          : 0;
