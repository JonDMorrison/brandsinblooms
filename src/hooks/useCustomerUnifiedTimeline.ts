    ...
    queryFn: async () => {
      if (!customerId) {
        return {
          data: [],
          error: null,
        } as CustomerDashboardSeriesResult<TimelineEvent>;
      }
      ...
      try {
        ...
        const filtered = ((data || []) as TimelineEvent[]).filter((event) =>
          isOnOrAfterRangeStart(event.created_at, startDate),
        );
...
export const getEventCategoryColor = (category: string): string => {
  const colors: Record<string, string> = {
    ...
    lifecycle:
      "bg-gray-100 text-gray-700 dark:bg-gray-950/50 dark:text-gray-300",
