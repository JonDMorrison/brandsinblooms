...
export function useImageGenerationQueue(
  options: UseImageGenerationQueueOptions,
) {
  ...
  const processQueue = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        `❌ [ImageQueue] Failed to generate image for block ${task.blockId}:`,
        error,
      );
  ...
  const enqueueImageGeneration = useCallback(
    (
      ...
      return new Promise((resolve, reject) => {
        ...
        const existingTask = queueRef.current.find(
          (t) => t.blockId === blockId,
        );
        if (existingTask) {
          return;
  ...
  const enqueueMultipleBlocks = useCallback(
    (blocks: ContentBlock[], campaignTitle: string) => {
      ...
      blocksNeedingImages.forEach((block) => {
        const contentContext =
          block.body || block.content || block.headline || campaignTitle;
