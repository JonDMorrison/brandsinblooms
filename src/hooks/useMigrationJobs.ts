...
export const useMigrationJobs = () => {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === "UPDATE") {
            ...
            if (job.source_platform === "mailchimp") {
              return;
            }
  ...
  return {
    ...
    activeJobs: jobs.filter(
      (j) => j.status === "running" && j.source_platform !== "mailchimp",
    ),
    pauseJob: (jobId: string) =>
      controlMutation.mutate({ jobId, action: "pause" }),
    resumeJob: (jobId: string) =>
      controlMutation.mutate({ jobId, action: "resume" }),
    cancelJob: (jobId: string) =>
      controlMutation.mutate({ jobId, action: "cancel" }),
