...
export const useUnsplash = () => {
  ...
  const getCuratedCollectionImages = useCallback(
    async (page = 1): Promise<ImageAttachment[]> => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const mappedImages = (data?.images || []).map(
          (img: UnsplashImage): ImageAttachment => ({
            id: img.id,
            ...
            photographer_url:
              ...
              `https://unsplash.com/@${img.photographer?.toLowerCase().replace(/\s+/g, "")}`,
  ...
  const searchImages = useCallback(
    async (query: string, useRawQuery = false): Promise<ImageAttachment[]> => {
  ...
  const refreshImages = useCallback(
    async (prevQuery: string): Promise<ImageAttachment[]> => {
