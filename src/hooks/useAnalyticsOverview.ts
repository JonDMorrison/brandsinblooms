    ...
    try {
      ...
      const [
        ...
        supabase
          ...
          .select(
            "impressions, reach, likes_count, comments_count, shares_count, engagement_rate",
          )
        ...
        // Previous period hub views for symmetric growth calculation
        supabase
          .from("hub_views")
          .select("id, campaign_id")
        ...
        supabase
          .from("post_performance")
          .select(
            "impressions, reach, likes_count, comments_count, shares_count",
          )
      ...
      if (postPerformanceResult.data) {
        const postImpressions = postPerformanceResult.data.reduce(
          (sum, p) => sum + (p.impressions || 0),
          0,
        );
        const postEngagement = postPerformanceResult.data.reduce(
          (sum, p) =>
            sum +
            (p.likes_count || 0) +
            (p.comments_count || 0) +
      ...
      const clicks =
        analyticsEventsResult.data?.filter((e) => e.event_type === "link_click")
          .length || 0;

      // Calculate conversions (coupon redemptions, purchase events)
      const conversions =
        analyticsEventsResult.data?.filter(
          (e) =>
      ...
      prevTotalViews += prevHubViewsResult.data?.length || 0;
      ...
      if (prevPostPerformanceResult.data) {
        prevTotalViews += prevPostPerformanceResult.data.reduce(
          (sum, p) => sum + (p.impressions || 0),
          0,
        );
      }
