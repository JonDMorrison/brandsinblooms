...
interface UseCampaignBouncesResult {
  ...
  suppressAll: () => void;
  toggleSuppression: (bounce: BouncedEmail, nextSuppressed: boolean) => void;
...
export function useCampaignBounces(
  ...
  const { mutate: suppressAll, isPending: isSuppressing } = useMutation({
    mutationFn: async () => {
      const unsuppressedEmails = bouncedEmails.filter(
        (e) => !e.isSuppressed && !!e.email,
      );
      ...
      const existingSet = new Set(
        existingSuppressions?.map((s) => s.email) || [],
      );
  ...
  const {
    mutate: toggleSuppression,
    isPending: isTogglingSuppression,
    variables: activeToggle,
  } = useMutation({
    mutationFn: async ({
      bounce,
      nextSuppressed,
      ...
      if (nextSuppressed) {
        ...
        return { email: bounce.email, nextSuppressed };
      }
    ...
    onSuccess: ({ email, nextSuppressed }) => {
      toast({
        title: nextSuppressed ? "Email suppressed" : "Suppression removed",
        description: nextSuppressed
          ? `${email} has been suppressed for future sends.`
          : `${email} has been removed from the suppression list.`,
      });
      queryClient.invalidateQueries({
        queryKey: ["campaign-bounces", campaignId],
      });
    ...
    onError: (error) => {
      toast({
        title: "Failed to update suppression",
        description: error.message,
        variant: "destructive",
      });
    },
  ...
  return {
    ...
    unsuppressedCount,
    suppressAll,
    toggleSuppression: (bounce, nextSuppressed) =>
      toggleSuppression({ bounce, nextSuppressed }),
    isSuppressing: isSuppressing || isTogglingSuppression,
    activeBounceId: activeToggle?.bounce.id ?? null,
  };
}
