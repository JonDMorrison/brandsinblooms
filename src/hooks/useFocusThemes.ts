...
const categorizeTheme = (
  theme: any,
): "plant_care" | "decor" | "sale" | "holidays" => {
...
const generateTags = (theme: any): string[] => {
  ...
  if (seasonal.includes("fall") || seasonal.includes("autumn"))
    tags.push("fall");
...
const transformTheme = (dbTheme: any, currentWeek: number): FocusTheme => {
  ...
  return {
    ...
    title: dbTheme.title,
    description:
      dbTheme.content_ideas ||
      dbTheme.theme ||
      "Seasonal gardening activities and tips",
    teaser:
      dbTheme.seasonal_focus ||
      dbTheme.target_audience_notes ||
      "Professional garden center guidance",
...
export const useFocusThemes = () => {
  ...
  const loadThemes = async () => {
    ...
    try {
      ...
      let transformedThemes = (masterThemes || []).map((theme) =>
        transformTheme(theme, currentWeek),
      );
      ...
      transformedThemes = transformedThemes.filter((theme) => {
        ...
        const adjustedWeekDiff =
          weekDifference < -26 ? weekDifference + 52 : weekDifference;
      ...
      let filteredThemes = transformedThemes.filter((theme) => {
        ...
        if (tenant?.id) {
          const userStatus = userThemeStatuses.find(
            (s) => s.theme_id === theme.id,
          );
          if (
            userStatus &&
            ["generated", "skipped"].includes(userStatus.status)
          ) {
        ...
        if (filters.timeCommitment.length > 0) {
          const timeMatch = filters.timeCommitment.some((time) => {
            if (time === "quick" && theme.timeToComplete.includes("30 minutes"))
              return true;
            if (time === "short" && theme.timeToComplete.includes("1 hour"))
              return true;
            if (time === "medium" && theme.timeToComplete.includes("2-3 hours"))
              return true;
            return false;
          });
  ...
  const skipTheme = async (themeId: string) => {
    ...
    try {
      await supabase.from("user_theme_status").upsert({
        user_id: user.id,
        tenant_id: tenant.id,
        theme_id: themeId,
        status: "skipped",
