...
export class AnalyticsTracker {
  ...
  static async getCampaignMetrics(campaignId: string, days: number = 30) {
    try {
      ...
      const metrics = {
        total_sent:
          events?.filter((e) => e.event_type === "sms_sent").length || 0,
        total_delivered:
          events?.filter((e) => e.event_type === "sms_delivered").length || 0,
        total_clicks:
          events?.filter((e) => e.event_type === "link_click").length || 0,
        total_redemptions:
          events?.filter((e) => e.event_type === "coupon_redeem").length || 0,
        total_revenue:
          events
            ?.filter((e) => e.event_type === "coupon_redeem")
            .reduce(
              (sum, e) => sum + ((e.payload as any)?.net_sales || 0),
              0,
            ) || 0,
        unique_clicks: new Set(
          events
            ?.filter((e) => e.event_type === "link_click")
            .map((e) => e.contact_id),
        ).size,
        ctr: 0,
        revenue_per_send: 0,
      ...
      metrics.ctr =
        metrics.total_sent > 0
          ? (metrics.unique_clicks / metrics.total_sent) * 100
          : 0;
      metrics.revenue_per_send =
        metrics.total_sent > 0 ? metrics.total_revenue / metrics.total_sent : 0;
