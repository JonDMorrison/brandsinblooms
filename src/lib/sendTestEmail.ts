...
export const ERROR_MESSAGES = {
  ...
  SENDER_NOT_CONFIGURED:
    "Email sender not configured. Please set up your email sender first.",
  SENDER_NOT_VERIFIED:
    "Email sender is not verified. Please verify your email domain.",
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message?.includes("not verified")) {
        return {
          success: false,
          message: ERROR_MESSAGES.SENDER_NOT_VERIFIED,
          error: error.message,
        };
      }
      if (error.message?.includes("not found")) {
        return {
          success: false,
          message: ERROR_MESSAGES.SENDER_NOT_CONFIGURED,
          error: error.message,
        };
      }
 ...
 */
export async function sendSenderTestEmail(
  payload: SenderTestPayload,
): Promise<TestEmailResponse> {
  try {
    const { data, error } = await supabase.functions.invoke("send-test-email", {
      body: payload,
 ...
 */
export async function sendDomainTestEmail(
  payload: DomainTestPayload,
): Promise<TestEmailResponse> {
  try {
    const { data, error } = await supabase.functions.invoke("send-test-email", {
      body: payload,
