...
export const triggerCatalog: TriggerMeta[] = [
  ...
  },
  {
    ...
    description:
      "Fires on payment.created or payment.updated (COMPLETED) or invoice.payment_made (PAID)",
    audienceType: "event",
  },
  {
  ...
  },
  {
    ...
    defaultDelayMin: 0,
    channels: ["sms", "email"],
    goalHint: "Lead Capture",
    description:
      "Triggered immediately after a tracked form submission. Select an optional source form in trigger settings.",
    audienceType: "event",
  },
  {
  ...
  },
  {
    ...
    goalHint: "Welcome Series",
    description:
      "Triggered when a contact is added to the Loyalty Members segment via POS sync or manual addition",
    audienceType: "event",
  },
  {
    id: "segment.added",
    ...
    description:
      "Triggered when a contact is added to a specific segment. Select the target segment after choosing this trigger.",
    audienceType: "event",
  },
  {
    id: "persona.assigned",
    ...
    description:
      "Triggered when a contact is assigned to a specific persona. Select the target persona after choosing this trigger.",
    audienceType: "event",
  },
];
...
export function getTriggerById(id: string): TriggerMeta | null {
  const resolvedId = triggerAliases[id] ?? id;
  return triggerCatalog.find((trigger) => trigger.id === resolvedId) || null;
}
