...
const createTestBlock = (
  overrides: Partial<ContentBlock> & { type: BlockType },
): ContentBlock => ({
  id: `test-${Date.now()}`,
...
describe("Block Field Mapping - Round Trip", () => {
  describe("normalizeBlockForSave", () => {
    ...
    it("should save email-safe hero backgroundImageUrl to image_url", () => {
      const heroBlock: ContentBlock = {
        id: "hero-1",
        type: "email-safe-hero",
        headline: "Hero headline",
        body: "Supporting copy",
        backgroundImageUrl: "https://cdn.bloomsuite.test/hero-bg.jpg",
        source: "manual",
      };

      const saved = normalizeBlockForSave(heroBlock, 0);

      expect(saved.block_type).toBe("email-safe-hero");
      expect(saved.image_url).toBe("https://cdn.bloomsuite.test/hero-bg.jpg");
      expect(saved.content.backgroundImageUrl).toBe(
        "https://cdn.bloomsuite.test/hero-bg.jpg",
      );
    });
  ...
  describe("normalizeBlockFromDatabase", () => {
    ...
    it("should load email-safe hero image_url into backgroundImageUrl", () => {
      const dbBlock: DatabaseBlock = {
        id: "hero-1",
        block_type: "email-safe-hero",
        content: {
          headline: "Hero headline",
          body: "Supporting copy",
        },
        image_url: "https://cdn.bloomsuite.test/hero-bg.jpg",
        order_index: 0,
        source: "manual",
      };
      ...
      expect(loaded.type).toBe("email-safe-hero");
      expect(loaded.backgroundImageUrl).toBe(
        "https://cdn.bloomsuite.test/hero-bg.jpg",
      );
      expect(loaded.imageUrl).toBeUndefined();
    });
  ...
  describe("Round-trip: save then load", () => {
    ...
    it("should handle multiple blocks in sequence", () => {
      ...
      expect(loadedBlocks[0].backgroundImageUrl).toBe(
  ...
  describe("isHeaderBlock helper", () => {
    it("should identify header blocks", () => {
      ...
      expect(isHeaderBlock({ type: "email-safe-hero" })).toBe(true);
