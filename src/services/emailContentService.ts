  ...
  const generationPromises = emailItems.map(async (item) => {
    try {
      ...
      if (emailContent) {
        ...
        const subjectQuality = assessContentQuality(
          emailContent.subject,
          "subject",
        );
        const preheaderQuality = assessContentQuality(
        ...
        if (
          subjectQuality.level === "poor" ||
          preheaderQuality.level === "poor"
        ) {
        ...
        const improvedCaption = item.caption
          ? formatEmailContent(sanitizeAndImproveContent(item.caption))
          : item.caption;
        return {
          ...item,
