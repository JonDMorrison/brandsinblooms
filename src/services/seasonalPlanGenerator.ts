...
export const getSeasonalThemesForMonth = async (
  month: string,
  offset = 0,
  limit = 6,
): Promise<{ themes: SeasonalPlanTheme[]; hasMore: boolean }> => {
  ...
  try {
    ...
    if (themes.length === 0 || offset === 0) {
      ...
      const monthSpecificThemes = [...fallbackThemes, ...holidayThemes].map(
        (theme, index) => ({
    ...
    const themes = [...fallbackThemes, ...holidayThemes].map(
      (theme, index) => ({
...
export const generateMultiThemeSeasonalPlanContent = async (
  ...
  try {
    ...
    console.error(
      "Failed to enhance emails with AI, continuing with basic content:",
      error,
    );
  }
  ...
  try {
    ...
    console.error(
      "Failed to enhance social posts with AI, continuing with basic content:",
      error,
    );
  }
...
const generateOverlayContent = async (
  ...
  const weekDate = new Date(
    firstDay.getTime() + (targetWeek - 1) * 7 * 24 * 60 * 60 * 1000,
  );
  ...
  const items: PlanItem[] = [
    // Email overlay
    {
      ...
      caption: generateSeasonalEmailContent(
        theme,
        monthName,
        seasonalFocus,
        contentIdeas[0],
    ...
    },
    {
      ...
      caption: generateInstagramContent(
        theme,
        monthName,
        seasonalFocus,
        contentIdeas[0],
...
export const generateSeasonalPlanContent = async (
  ...
  const week4 = new Date(
    Math.min(
      firstDay.getTime() + 28 * 24 * 60 * 60 * 1000,
      new Date(year, monthDate.getMonth() + 1, 0).getTime(),
    ),
  );

  // Helpers to land on specific weekdays within a week block
  const dayMs = 24 * 60 * 60 * 1000;
  const fridayOfWeek2 = new Date(
    week2.getTime() + ((5 - week2.getDay() + 7) % 7) * dayMs,
  ); // 5 = Friday
  const saturdayOfWeek3 = new Date(
    week3.getTime() + ((6 - week3.getDay() + 7) % 7) * dayMs,
  ); // 6 = Saturday
  ...
  const items: PlanItem[] = [
    // Monthly blog post with enhanced content
    {
      ...
      enhancedContent: generateEnhancedBlogContent(
        theme,
        monthName,
        seasonalFocus,
        contentIdeas[0],
    ...
    },
    {
      ...
      caption: generatePromotionalContent(
        theme,
        monthName,
        seasonalFocus,
        holidays,
...
const generateSeasonalEmailContent = (
  theme: SeasonalPlanTheme,
  month: string,
  ...
  const holidayContext =
    holidays && holidays.length > 0
      ? ` Perfect timing for ${holidays[0].holiday_name}!`
      : "";
...
const generatePromotionalContent = (
  theme: SeasonalPlanTheme,
  month: string,
  seasonalFocus: string,
  holidays?: any[],
  ...
  const holidayOffer =
    holidays && holidays.length > 0
      ? `${holidays[0].holiday_name} Special: `
      : "";
...
const generateSocialContent = (
  ...
  const randomContent =
    socialContent[Math.floor(Math.random() * socialContent.length)];
  const engagement =
    platform === "facebook"
      ? "  What gardening goals are you tackling this month?"
      : "";
...
const generateBehindScenesContent = (
  ...
  const randomInsight =
    expertInsights[Math.floor(Math.random() * expertInsights.length)];
  return `${randomInsight}  Visit us to see the difference expert selection makes for your garden.`;
};

const generateCustomerSpotlightContent = (
  theme: SeasonalPlanTheme,
  month: string,
) => {
...
const generateTransformationContent = (
  theme: SeasonalPlanTheme,
  month: string,
  seasonalFocus: string,
) => {
...
const generateWorkshopContent = (
  ...
  const randomWorkshop =
    workshopContent[Math.floor(Math.random() * workshopContent.length)];
...
const generateBlogContent = (
  ...
  const holidayContext =
    holidays && holidays.length > 0
      ? ` Plus, discover how to make the most of ${holidays[0].holiday_name} with your garden.`
      : "";
...
const getMonthBasedThemes = (monthDate: Date): SeasonalPlanTheme[] => {
  ...
  if (month >= 2 && month <= 4) {
    ...
    return [
      {
        id: "spring-planting",
        label: "Spring Planting Festival",
        description:
          "Early season planting and garden preparation for spring growth",
      },
      {
        id: "seed-starting",
        label: "Seed Starting Workshop",
        description:
          "Indoor seed starting and transplant preparation techniques",
      },
      {
        id: "spring-cleanup",
        label: "Spring Garden Renewal",
        description:
          "Garden cleanup, pruning, and soil preparation for the growing season",
      },
      {
        id: "pollinator-garden",
        label: "Pollinator Garden Design",
        description: "Creating bee and butterfly-friendly garden spaces",
      },
    ];
    ...
    return [
      {
        id: "summer-care",
        label: "Summer Garden Mastery",
        description:
          "Heat protection, watering strategies, and summer plant care",
      },
      {
        id: "harvest-preservation",
        label: "Harvest & Preservation",
        description: "Harvesting techniques and preserving summer abundance",
      },
      {
        id: "container-gardening",
        label: "Container Garden Magic",
        description:
          "Maximizing small spaces with container gardening solutions",
      },
      {
        id: "drought-resistant",
        label: "Drought-Resistant Gardens",
        description: "Water-wise gardening with drought-tolerant plants",
      },
    ];
    ...
    return [
      {
        id: "fall-planting",
        label: "Fall Planting Success",
        description: "Fall planting strategies and winter preparation",
      },
      {
        id: "autumn-color",
        label: "Autumn Color Spectacular",
        description:
          "Designing for fall color with trees, shrubs, and perennials",
      },
      {
        id: "bulb-planting",
        label: "Spring Bulb Planning",
        description: "Planting spring-flowering bulbs for next year's garden",
      },
      {
        id: "harvest-festival",
        label: "Harvest Festival Celebration",
        description: "Celebrating the fall harvest with seasonal decorations",
      },
    ];
  } else {
    ...
    return [
      {
        id: "houseplant-care",
        label: "Houseplant Winter Care",
        description: "Indoor plant care and winter gardening techniques",
      },
      {
        id: "holiday-gardening",
        label: "Holiday Garden Magic",
        description: "Holiday decorations, plants, and seasonal arrangements",
      },
      {
        id: "winter-planning",
        label: "Garden Planning Season",
        description:
          "Planning and designing next year's garden layout and goals",
      },
      {
        id: "indoor-growing",
        label: "Indoor Growing Systems",
        description: "Year-round growing with indoor gardening setups",
      },
    ];
  }
};
    ...
    1: [
      {
        ...
        label: "New Year Garden Goals",
        description:
          "Setting garden resolutions and planning for the year ahead",
      },
    ],
    2: [
      {
        id: "valentines-flowers",
        label: "Valentine's Day Blooms",
        description:
          "Romantic flowers and plants perfect for Valentine's Day gifts",
      },
    ],
    3: [
      {
        id: "spring-equinox",
        label: "Spring Equinox Celebration",
        description:
          "Welcoming spring with seasonal planting and garden awakening",
      },
    ],
    4: [
      {
        id: "easter-gardens",
        label: "Easter Garden Revival",
        description: "Spring gardens and Easter-themed planting ideas",
      },
      {
        id: "earth-day",
        label: "Earth Day Gardening",
        description: "Sustainable gardening practices for Earth Day and beyond",
      },
    ],
    5: [
      {
        id: "mothers-day-garden",
        label: "Mother's Day Garden Gifts",
        description:
          "Beautiful plants and garden gifts perfect for celebrating mothers",
      },
    ],
    6: [
      {
        id: "fathers-day-garden",
        label: "Father's Day Garden Projects",
        description: "Garden tools, projects, and plants that dads will love",
      },
    ],
    7: [
      {
        id: "july-fourth-garden",
        label: "July 4th Patriotic Gardens",
        description:
          "Red, white, and blue themed garden designs and celebrations",
      },
    ],
    8: [
      {
        id: "late-summer-harvest",
        label: "Late Summer Abundance",
        description:
          "Maximizing late summer harvests and preserving techniques",
      },
    ],
    9: [
      {
        id: "fall-equinox",
        label: "Fall Equinox Transition",
        description: "Transitioning gardens from summer to fall growing season",
      },
    ],
    10: [
      {
        id: "halloween-gardens",
        label: "Halloween Harvest Decor",
        description: "Pumpkins, gourds, and spooky garden decorations",
      },
    ],
    11: [
      {
        id: "thanksgiving-harvest",
        label: "Thanksgiving Garden Gratitude",
        description:
          "Celebrating the harvest season with gratitude and abundance",
      },
    ],
    12: [
      {
        id: "holiday-plants",
        label: "Holiday Plant Traditions",
        description:
          "Poinsettias, Christmas trees, and festive holiday plant care",
      },
      {
        id: "winter-solstice",
        label: "Winter Solstice Gardens",
        description:
          "Embracing the longest night with evergreen beauty and winter interest",
      },
    ],
  };
