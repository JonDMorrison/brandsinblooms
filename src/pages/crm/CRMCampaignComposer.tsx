...
const CRMCampaignComposer = () => {
  ...
  const [selectedSegmentPersona, setSelectedSegmentPersona] =
  ...
  const [originalNewsletterContent, setOriginalNewsletterContent] =
    useState("");
  ...
  const processNewsletterParams = async (
    ...
    try {
      const result = await convertNewsletterToCRM(
        contentTaskId,
        title,
        content,
      );
      ...
      toast({
        ...
        description:
          "Your newsletter has been converted to an email campaign. Review and customize before sending.",
      });
    } catch (error) {
      ...
      toast({
        ...
        description:
          "There was an issue converting your newsletter. Please try again.",
        variant: "destructive",
      });
    } finally {
...
Write a 75-word email using the persona's tone and style: ${aiPrompt}
  ...
  const sendNow = async () => {
    ...
    try {
      ...
      if (insertError || !inserted) {
        throw insertError || new Error("Failed to create campaign");
      }
      ...
      if (claimError || !claimData?.[0]?.success) {
        throw new Error(
          claimData?.[0]?.error_message ||
            claimError?.message ||
            "Failed to claim campaign for send",
        );
      }
  ...
  const handleSuggestionSelect = (suggestion: CampaignTemplate) => {
    ...
    toast({
      ...
      description:
        "AI prompt loaded! Click 'Generate Content' to create your campaign.",
    });
  };
        ...
        <div className="container mx-auto p-6 space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  {isFromNewsletter
                    ? "Newsletter converted to email campaign"
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <Dialog
                        open={showSubjectSuggestions}
                        onOpenChange={setShowSubjectSuggestions}
                      >
                          ...
                          <div className="space-y-4">
                            <div className="space-y-2">
                              <Label htmlFor="subject-topic">
                                What's this email about? (optional)
                              </Label>
                              <Input
                                id="subject-topic"
                                value={subjectTopic}
                                onChange={(e) =>
                                  setSubjectTopic(e.target.value)
                                }
                                placeholder="e.g., Spring planting tips, summer watering guide..."
                              />
                              <p className="text-sm text-muted-foreground">
                                Leave blank to use your email content for
                                suggestions
                              </p>
                            </div>
                          ...
                          <div className="border rounded-lg p-4 bg-white">
                            <div className="border-b pb-4 mb-4">
                              ...
                              <div className="font-semibold">
                                {formData.subject_line || "No subject line"}
                              </div>
                            </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      ...
                      <div className="text-sm text-muted-foreground">
                        {selectedSegment.customer_count} customers will receive
                        this email
                      </div>
                    </div>
                  </CardContent>
                ...
                <CardContent className="space-y-3 text-sm">
                  <div>
                    ...
                    <div className="text-muted-foreground">
                      Use emojis and keep under 50 characters
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="text-muted-foreground">
                      Include seasonal tips and plant care advice
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              {selectedSegmentPersona && (
                <Card>
                  ...
                  <CardContent className="space-y-3 text-sm">
                    <div>
                      <div className="font-medium">
                        {selectedSegmentPersona.name}
                      </div>
                      <div className="text-muted-foreground">
                        {selectedSegmentPersona.description}
                      </div>
                    </div>
                    <div>
                      <div className="font-medium">Writing Tone:</div>
                      <div className="text-muted-foreground">
                        {selectedSegmentPersona.tone}
                      </div>
                    </div>
                    {selectedSegmentPersona.buying_triggers &&
                      selectedSegmentPersona.buying_triggers.length > 0 && (
                        <div>
                          ...
                          <div className="text-muted-foreground">
                            {selectedSegmentPersona.buying_triggers
                              .slice(0, 2)
                              .join(", ")}
                            {selectedSegmentPersona.buying_triggers.length >
                              2 && "..."}
                          </div>
                        </div>
                      )}
          ...
          <PersonaAISuggestionsModal
            ...
            onPersonaSelect={(persona, theme) => {
              ...
              setAiPrompt(
                `Write an email about ${theme || "gardening tips"} for ${persona.name} customers who are ${persona.description.toLowerCase()}. Use a ${persona.tone} tone.`,
              );
              setShowAIModal(true);
            }}
          />
