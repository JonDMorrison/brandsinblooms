...
const CRMCampaignBuilderInner: React.FC<CRMCampaignBuilderProps> = ({
  onSwitchToSimple,
}) => {
  ...
  useEffect(() => {
    ...
    const changedBlocks = blocks.filter((block) => {
      ...
      return (
        JSON.stringify(block.content) !== JSON.stringify(prevBlock.content) ||
      ...
      data?.map((block) => {
        ...
        const processedContent = {
          ...
          backgroundImageUrl:
            ...
            block.block_type === "newsletter-header"
              ? block.image_url
              : "") ||
            "",
  ...
  const generateEmailHTML = () => {
    ...
    return `
      <div style="font-family: ${globalSettings.fontFamily}; font-size: ${globalSettings.fontSize}; width: 100%;">
        ${htmlContent}
      </div>
    `;
  };
  ...
  const handleSwitchToSimple = () => {
    if (blocks.length > 0) {
      if (
        confirm(
          "Switching to Simple mode will remove all blocks and convert to a plain text email. Continue?",
        )
      ) {
        onSwitchToSimple?.();
      }
  ...
  const onboardingSteps = [
    ...
    },
    {
      ...
      title: "Preview & Send",
      description:
        "View your campaign on mobile or desktop, then schedule or send.",
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            Please upgrade your plan to access the email campaign builder.
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex-1 flex">
          ...
          <div className="flex-1 p-8 bg-muted/10">
            {previewMode === "mobile" ? (
              <MobilePreviewFrame>
                {blocks.length === 0 ? (
                  <div className="p-8 text-center text-muted-foreground">
                    ...
                    <p className="text-sm">
                      Add blocks to see how your email looks on mobile
                    </p>
                  </div>
                ) : (
              ...
              <div
                className={`mx-auto bg-white shadow-xl rounded-lg border max-w-2xl`}
              >
