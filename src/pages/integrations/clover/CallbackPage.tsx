...
const CallbackPage = () => {
  ...
  const [message, setMessage] = useState('Processing your Clover connection...');
    ...
    const handleCallback = async () => {
      ...
      try {
        ...
        if (!code || !merchantId) {
          ...
          setStep('Missing authorization code or merchant ID');
        ...
        const response = await fetch(`${supabaseUrl}/functions/v1/clover-oauth-callback`, {
          method: 'POST',
          headers: {
        ...
        if (fnError || data?.error || !data?.success) {
          console.error('[CLOVER-Callback] Edge function error:', fnError || data);
          setStatus('error');
  ...
  const broadcastResult = (result: any) => {
    ...
    if (window.opener && !window.opener.closed) {
      try {
        window.opener.postMessage({ type: 'clover_oauth_result', data: result }, window.location.origin);
      } catch (e) {
