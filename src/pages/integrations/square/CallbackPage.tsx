...
const CallbackPage = () => {
  ...
  const [message, setMessage] = useState('Processing your Square connection...');
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        ...
        const response = await fetch(`${supabaseUrl}/functions/v1/square-oauth-callback`, {
          method: 'POST',
          headers: {
        ...
        if (fnError || data?.error || !data?.success) {
          console.error('[SQUARE-Callback] Edge function error:', fnError || data);
          setStatus('error');
  ...
  const broadcastResult = (result: any) => {
    ...
    if (window.opener && !window.opener.closed) {
      try {
        window.opener.postMessage({ type: 'square_oauth_result', data: result }, window.location.origin);
      } catch (e) {
