...
export const createBlocksFromPlainText = (
  content: string,
  ...
  if (content.includes("blocks:") && content.includes("- title:")) {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith("- title:")) {
        ...
        currentBlock.image_prompt = trimmed
          .replace("image_prompt:", "")
          .replace(/"/g, "")
          .trim();
      } else if (trimmed.startsWith("alt_text:")) {
        currentBlock.alt_text = trimmed
          .replace("alt_text:", "")
          .replace(/"/g, "")
          .trim();
      }
...
export const generateIntroFromContent = (
  content: string,
  campaignTitle?: string,
): string => {
  if (!content) return "";
  ...
  const startIndex = lines[0]?.match(/^#+|^\*\*.*\*\*$|^[A-Z][A-Z\s]{5,50}:?$/)
    ? 1
    : 0;
  ...
  for (let i = startIndex; i < lines.length; i++) {
    ...
    if (
      ...
      return (
        line.replace(/\*\*(.*?)\*\*/g, "$1").substring(0, 200) +
        (line.length > 200 ? "..." : "")
      );
    }
  }
  ...
  return campaignTitle
    ? `Discover this week's ${campaignTitle.toLowerCase()} insights and tips.`
    : "";
};
...
export const checkIsPlaceholderContent = (content: string): boolean => {
  ...
  // Only check for truly empty or placeholder content - be less restrictive
  ...
  // Check if content is too short (less than 100 characters to be more lenient)
...
export const checkIsUnstructuredNewsletter = (content: string): boolean => {
  ...
  // Only mark as unstructured if it's truly raw email format
