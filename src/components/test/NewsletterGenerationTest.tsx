  ...
  const testNewsletterGeneration = async () => {
    ...
    try {
      ...
      if (parsed && parsed.blocks && parsed.blocks.length === 4) {
        ...
        toast.warning(
          "⚠️ Newsletter generated but may not have proper 4-section structure",
        );
      }
    } catch (error) {
      ...
      <div className="mb-6">
        ...
        {!user && (
          <p className="text-sm text-gray-500 mt-2">
            Please log in to test newsletter generation
          </p>
        )}
      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div className="p-3 bg-gray-50 rounded">
                ...
                {parsedNewsletter ? "✅ Yes" : "❌ No"}
              </div>
              <div className="p-3 bg-gray-50 rounded">
                <strong>Sections Count:</strong>{" "}
                {parsedNewsletter?.blocks?.length || 0}
              </div>
              <div className="p-3 bg-gray-50 rounded">
                <strong>Has YAML Structure:</strong>{" "}
                {generatedContent.includes("newsletter_md:")
                  ? "✅ Yes"
                  : "❌ No"}
              </div>
              <div className="p-3 bg-gray-50 rounded">
