...
interface PublishTaskEdgeResponse {
  success?: boolean;
  message?: string;
  result?: {
    status?: string;
...
export const PostToSocialButton: React.FC<PostToSocialButtonProps> = ({
    ...
    const fetchConnections = async () => {
      ...
      try {
        ...
        const validConnections = (data || []).filter(
          (connection): connection is SocialConnection =>
  ...
  const processContentForPlatform = (
    content: string,
    platform: "facebook" | "instagram",
  ) => {
    ...
    // Limit hashtags
    const hashtagArray = finalHashtags
      .split(" ")
  ...
  const handlePost = async (platform: "facebook" | "instagram") => {
    ...
    try {
      ...
      const processedContent = processContentForPlatform(
        task.ai_output,
        platform,
      );
      ...
      try {
        ...
        const typedResponse = (responseData || {}) as PublishTaskEdgeResponse;
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-3">
                            <PlatformIcon
                              className={`w-5 h-5 ${connection.platform === "facebook" ? "text-blue-600" : "text-pink-500"}`}
                            />
                            <div>
