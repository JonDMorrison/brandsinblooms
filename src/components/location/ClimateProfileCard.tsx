      ...
      <Stack spacing={2.5}>
        ...
        >
          <Stack spacing={1}>
            <Skeleton
              animation="wave"
              sx={{ width: 150, height: 18 }}
              variant="text"
            />
            <Skeleton
              animation="wave"
              sx={{ width: 240, height: 14 }}
              variant="text"
            />
          </Stack>
          <Skeleton
        ...
        <Sheet variant="soft" sx={{ p: 2.5, borderRadius: "lg" }}>
          <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
            <Skeleton
              animation="wave"
              sx={{ width: 48, height: 48, borderRadius: "xl", flexShrink: 0 }}
              variant="rectangular"
            />
            <Stack spacing={1} sx={{ flex: 1 }}>
              <Skeleton
                animation="wave"
                sx={{ width: 180, height: 20 }}
                variant="text"
              />
              <Skeleton
                animation="wave"
          ...
          {Array.from({ length: 3 }).map((_, index) => (
            <Sheet key={index} variant="soft" sx={{ p: 2, borderRadius: "lg" }}>
              <Stack spacing={0.75}>
                <Skeleton
                  animation="wave"
                  sx={{ width: 84, height: 12 }}
                  variant="text"
                />
                <Skeleton
                  animation="wave"
                  sx={{ width: 96, height: 18 }}
                  variant="text"
                />
              </Stack>
            </Sheet>
...
export const ClimateProfileCard: React.FC<ClimateProfileCardProps> = ({
  ...
  const resolvedClimateLabel =
    climateLabel || formatArchetypeLabel(climateArchetype);
      ...
      <Stack spacing={2.5}>
        {hasClimateData ? (
          <>
            ...
            >
              <Stack spacing={0.5}>
                ...
                <Typography level="body-sm" sx={{ color: "neutral.500" }}>
                  Derived from your confirmed location to guide seasonal and
              ...
              <JoyButton
                ...
                startDecorator={
                  !isRefreshing ? <RefreshCw className="h-4 w-4" /> : undefined
                }
                variant="plain"
              >
            ...
            >
              {usdaZone && <MetricCard label="USDA Zone" value={usdaZone} />}

              {hasFrostDates && (
                <>
                  <MetricCard
                    label="Last Frost"
                    value={formatFrostDate(lastFrostDate)}
                  />
                  <MetricCard
                    label="First Frost"
                    value={formatFrostDate(firstFrostDate)}
                  />
                </>
              )}
            </Box>
