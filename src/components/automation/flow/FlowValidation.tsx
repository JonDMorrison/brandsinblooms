    ...
    <div className="space-y-2">
      {validationResults.map((result, index) => (
        ...
        >
          ...
          />
          <AlertDescription
            className={result.type === "warning" ? "text-yellow-800" : ""}
          >
            {result.message}
          </AlertDescription>
        </Alert>
...
function validateFlow(
  ...
  if (actionNodes.length === 0) {
    results.push({
      type: "error",
      message:
        "Automation must have at least one action (email, SMS, or delay).",
    });
  }
  ...
  if (
    ...
    results.push({
      type: "error",
      message:
        "Please select target audience (personas or segments) before launching.",
    });
  } else if (selectedAudience.totalContacts === 0) {
    results.push({
      type: "warning",
      message:
        "Selected audience has 0 contacts. Automation will not send to anyone.",
    });
  }
  ...
  const handleFixClick = (issue: {
    ...
    if (issue.message.includes("must have a trigger node")) {
      ...
      const emailNode = nodes.find(
        (node) =>
          node.type === "email" && (!node.data?.subject || !node.data?.content),
      );
      if (emailNode) {
      ...
      const smsNode = nodes.find(
        (node) => node.type === "sms" && !node.data?.message,
      );
      if (smsNode) {
