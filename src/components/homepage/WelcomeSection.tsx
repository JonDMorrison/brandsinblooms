  ...
  useEffect(() => {
    const fetchBusinessName = async () => {
      ...
      try {
        ...
        if (error) {
          console.error(
            "WelcomeSection: Error fetching company profile:",
            error,
          );
        ...
        console.error(
          "WelcomeSection: Unexpected error in fetchBusinessName:",
          error,
        );
    ...
    <div className="space-y-6 apple-fade-in">
      ...
      <div className="text-center">
        <div className="flex items-center justify-center gap-3 mb-4 apple-slide-up">
          <span className="seasonal-emoji text-3xl apple-hover-subtle">
            {welcomeMessage.emoji || seasonal.emoji}
          </span>
        ...
        <BodyLarge className="text-text-secondary mb-6 max-w-4xl mx-auto leading-relaxed apple-slide-up apple-stagger-1">
          {isCurrentUserSuperAdmin ? (
            <>
              Your super admin dashboard for managing the BloomSuite platform
              and all user accounts.
            </>
          ) : (
            <>
              Your AI-powered gardening assistant is ready to help you create
              engaging content that grows{" "}
