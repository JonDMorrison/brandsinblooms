...
export const generatePersonalizedContent = async (
  ...
  try {
    const { data: contentResult, error: contentError } =
...
export const generateCampaignContent = async (
  ...
  try {
    ...
    if (!tenantId) {
      console.error(
        "❌ [CRITICAL] tenantId is missing - multi-tenant isolation broken!",
        {
          campaignId,
          userId,
          campaignTheme,
    ...
    return apiDeduplicator.dedupe(cacheKey, async () => {
      ...
      if (!existingTasks) {
        ...
        if (checkError) {
          throw new Error(
            `Failed to check existing content: ${checkError.message}`,
          );
        }
