...
export const WeeklyThemeCarousel = ({
  ...
  const [reviewingCampaignId, setReviewingCampaignId] = useState<string | null>(
    null,
  );
  ...
  const [recentlyGeneratedTheme, setRecentlyGeneratedTheme] = useState<
    string | null
  >(null);
  ...
  const getThemeContentTasks = (theme?: WeeklyTheme) => {
    ...
    if (theme.campaignId) {
      const campaignTasksWithContent = tasks.filter(
        (task) =>
    ...
    const fallbackTasks = tasks.filter((task) => {
      ...
      const taskCampaign = tasks.find(
        (t) => t.campaign_id === task.campaign_id,
      );
      ...
      return titleWords.some(
        (word) => word.length > 3 && campaignTitle.includes(word),
      );
  ...
  const handleGenerateContent = async (theme?: WeeklyTheme) => {
    ...
    try {
      ...
      if (
        currentCampaign &&
        targetTheme.weekNumber === currentCampaign.week_number
      ) {
    ...
    if (themeTasks.length === 0) {
      console.error(
        "No tasks with content available for current theme:",
        currentTheme?.title,
      );
      toast({
        title: "Error",
  ...
  const getCategoryColor = (category: string) => {
    ...
    return (
      colors[category as keyof typeof colors] || "bg-gray-200 text-gray-900"
    );
  };
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-6 md:gap-8 items-center">
                          ...
                          <div className="flex justify-center md:justify-start">
                            <div className="relative w-40 h-40">
                              <div className="glass-coin-enhanced w-40 h-40 flex items-center justify-center group cursor-pointer animate-float">
                                {(() => {
                                  const iconMap = getFocusThemeIcon(
                                  ...
                                  const IconComponent =
                                    ...
                                    "icon" in iconMap
                                      ...
                                      : Sprout;
                                  return (
                                    <IconComponent className="w-20 h-20 text-white drop-shadow-xl group-hover:scale-125 transition-all duration-300" />
                                  );
                                })()}
                              ...
                              {(generatingTheme === currentTheme.id ||
                                (currentTheme.isCurrentWeek &&
                                  isGenerating)) && (
                                <svg
                                  className="absolute inset-0 w-40 h-40 progress-ring"
                                  aria-hidden="true"
                                >
                                  <circle
                                    cx="80"
                          ...
                          <div className="space-y-4 text-center md:text-left">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start items-center">
                              {generatingTheme === currentTheme.id ||
                              (currentTheme.isCurrentWeek && isGenerating) ? (
                                ...
                                <>
                                  ...
                                  {hasCurrentThemeContent ? (
                                    ...
                                    <Button
                                      onClick={() =>
                                        handleGenerateContent(currentTheme)
                                      }
