...
export const updateVideoTasksWithNewScript = async (
  campaignId: string,
...
export const createMissingTasks = async (
  campaignId: string,
  missingTypes: string[],
  campaignTitle: string,
  userId?: string,
  ...
  try {
    ...
    const actuallyMissingTypes = missingTypes.filter(
    ...
    for (let i = 0; i < actuallyMissingTypes.length; i++) {
      ...
      try {
        ...
        console.error(
          `❌ Error generating ${postType} content with OpenAI:`,
          error,
        );
        throw new Error(
          `Failed to generate ${postType} content. Please ensure OpenAI API key is configured.`,
        );
      }
