...
export const ReviewQueue = ({
  ...
  const tasksByTheme = tasksArray.reduce(
    (acc: Record<string, ContentTask[]>, task: ContentTask) => {
  ...
  const hasGeneratedContent = Object.keys(tasksByTheme).some(
    (theme) => theme !== "Business Content",
  );
  const businessTasks: ContentTask[] = Array.isArray(
    tasksByTheme["Business Content"],
  )
    ? tasksByTheme["Business Content"]
    : [];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm text-blue-800">
            ...
            <p>
              All content below requires your approval before it appears in
              "Ready to Post".
            </p>
          </div>
        </div>
      ...
      {hasGeneratedContent &&
        Object.entries(tasksByTheme)
          .filter(([theme]) => theme !== "Business Content")
          .map(([theme, tasks]: [string, ContentTask[]]) => (
            <div
              key={theme}
              className="border border-green-200 rounded-lg p-4 bg-white shadow-sm"
            >
