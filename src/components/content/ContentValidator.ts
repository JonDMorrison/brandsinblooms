...
export class ContentValidator {
  ...
  static async validateAndRegenerate(
    ...
    while (attempts < maxAttempts) {
      ...
      if (validation.isValid) {
        ...
        const finalContent = this.applyContentTypeCleaning(
          currentContent,
          contentType,
        );
        return {
    ...
    const cleanedContent = this.attemptComprehensiveCleanup(
      currentContent,
      contentType,
    );
  ...
  private static applyContentTypeCleaning(
    content: string,
    contentType?: string,
  ): string {
  ...
  private static attemptComprehensiveCleanup(
    content: string,
    contentType?: string,
  ): string {
    let cleaned = content;
