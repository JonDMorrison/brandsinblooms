...
const ActivityItem = ({ activity }: { activity: RecentActivity }) => {
  ...
  const getActivityMessage = () => {
    switch (activity.type) {
      ...
      case "failed":
        return `Failed to publish to ${activity.platform}`;
    }
  };
    ...
    try {
      ...
      if (!publishedError && published) {
        ...
        const scheduledThisWeek =
          scheduled?.filter((s) => isThisWeek(new Date(s.publish_at))).length ||
          0;
        ...
        const activity: RecentActivity[] = published
          .slice(0, 10)
          .map((post) => ({
            id: post.id,
              ...
              <div className="divide-y">
                {recentActivity.length === 0 ? (
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm">
                      Publish content to see activity here
                    </p>
                  </div>
                ) : (
