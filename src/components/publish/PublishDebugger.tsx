...
export const PublishDebugger = ({
  isVisible,
  onClose,
}: PublishDebuggerProps) => {
  const { user } = useAuth();
  ...
  const testConnections = async () => {
    ...
    try {
      ...
      addLog(
        "success",
        "connection",
        `Found ${data.length} active connections`,
        data,
      );
      ...
      for (const connection of data) {
        ...
        if (isExpired) {
          addLog(
            "warning",
            "connection",
            `Connection ${connection.platform} token expired`,
            connection,
          );
        } else {
          addLog(
            "success",
            "connection",
            `Connection ${connection.platform} is valid`,
          );
        }
      }
    } catch (error) {
  ...
  const testContent = async () => {
    ...
    try {
      ...
      addLog(
        "success",
        "database",
        `Found ${data.length} approved posts`,
        data,
      );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Diagnose publishing issues and test connections
              </p>
            </div>
          </div>
