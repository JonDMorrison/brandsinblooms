...
export const Homepage = () => {
  ...
  // Auto-generate content for current week campaign
  const autoGenerateCurrentWeekContent = async () => {
    if (!user || hasAutoGenerated) return;
    
    const weekNumber = getCurrentWeekNumber();
    const currentCampaign = campaigns.find(c => c.week_number === weekNumber);
    
    if (currentCampaign) {
      // Check if this campaign already has content
      ...
      if (!existingTasks || existingTasks.length === 0) {
        console.log('Auto-generating content for current week campaign:', currentCampaign.title);
        try {
          await generateRequiredTasks(currentCampaign.id, campaigns, user.id, handleTaskUpdate);
          toast.success("Welcome! We've generated this week's content for you to review.");
  ...
  useEffect(() => {
    if (campaigns.length > 0 && user) {
      ...
      // Auto-generate content for current week after ensuring campaigns exist
      autoGenerateCurrentWeekContent();
    }
