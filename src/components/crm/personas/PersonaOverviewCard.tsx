...
export const PersonaOverviewCard: React.FC<PersonaOverviewCardProps> = ({
  ...
  return (
    <Card className="h-full mobile-hover-lift mobile-card">
      <CardContent
        className={`${isMobile ? "p-4" : "p-6"} mobile-space-normal`}
      >
        {/* Header section with icon and title */}
        <div
          className={`flex ${isMobile ? "flex-col gap-3" : "items-start justify-between"} mb-4`}
        >
          <div className="flex items-center gap-3">
            <div className="p-2 bg-primary/10 rounded-lg mobile-touch-target">
              <IconComponent
                className={`${isMobile ? "mobile-icon-md" : "h-5 w-5"} text-primary`}
              />
            </div>
            <div className="flex-1 min-w-0">
              <h3
                className={`font-semibold ${isMobile ? "mobile-text-subheading" : "text-base"} mobile-prevent-overflow`}
              >
                {name}
              </h3>
            </div>
        ...
        {/* Description */}
        <p
          className={`${isMobile ? "mobile-text-body" : "text-sm"} text-muted-foreground mb-4 line-clamp-2 mobile-text-balance`}
        >
          {description}
        </p>
        ...
        <div className={`${isMobile ? "space-y-3 w-full" : "space-y-3"}`}>
          ...
          {loading ? (
            <div className="flex items-center gap-2">
              <Skeleton
                className={`${isMobile ? "mobile-icon-sm" : "h-4 w-4"} rounded-full`}
              />
            ...
            <div className="flex items-center gap-2">
              <Users
                className={`${isMobile ? "mobile-icon-sm" : "h-4 w-4"} text-muted-foreground`}
              />
              <span
                className={`${isMobile ? "mobile-text-caption" : "text-sm"} font-medium`}
              >
          ...
          {/* Action buttons */}
          <div
            className={`flex ${isMobile ? "flex-col gap-2 w-full" : "flex-col sm:flex-row gap-2"}`}
          >
            <Button
              variant="outline"
