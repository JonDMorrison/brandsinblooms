...
export function PersonaLibrary({ onClose, onPersonaSelect, customerId }: PersonaLibraryProps) {
  ...
  const modalContent = (
    <>
      {isLoading && (
        <div 
          className="fixed inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-[10000]"
          role="dialog" 
          aria-modal="true" 
        ...
        >
          <div className="bg-card p-6 rounded-lg border shadow-lg">
            ...
            <p id="loading-title" className="sr-only">Loading personas</p>
          </div>
        </div>
      )}
      ...
      {!isLoading && (
        <div 
          className="fixed inset-0 bg-background/90 backdrop-blur-sm flex items-center justify-center z-[10000] p-2 sm:p-4"
          role="dialog" 
          aria-modal="true" 
