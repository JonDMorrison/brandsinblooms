...
export const PostToCRMButton: React.FC<PostToCRMButtonProps> = ({
  ...
  const getTooltipMessage = () => {
    ...
    if (!hasCRMAccess)
      return "CRM features are not available on your current plan";
    if (!task.ai_output)
      return "Content must be generated first before creating a CRM campaign";
    if (task.status !== "approved")
      return "Content must be approved before creating a CRM campaign";
    if (task.post_type !== "newsletter")
      return "CRM campaigns are only available for newsletter content";
