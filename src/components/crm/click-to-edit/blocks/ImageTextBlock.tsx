...
export const ImageTextBlock: React.FC<ImageTextBlockProps> = ({
  ...
  const isImageLeft =
    block.layout === "image-left" || block.layout === "two-column-left";
  const isImageRight =
  ...
  const isTextOnly =
    !block.imageUrl && (block.layout === "full-width" || block.type === "text");
    ...
    try {
      ...
      if (hasContent) {
        ...
        if (month >= 2 && month <= 4) {
          ...
          season = "spring";
          seasonalElements =
            "blooming flowers, fresh green leaves, tulips, daffodils, cherry blossoms, vibrant spring colors";
        } else if (month >= 5 && month <= 7) {
          // June, July, August - Summer
          season = "summer";
          seasonalElements =
            "lush gardens, colorful flowers in full bloom, roses, hydrangeas, bright sunlight, verdant leaves";
        } else if (month >= 8 && month <= 10) {
          // September, October, November - Fall/Autumn
          season = "autumn";
          seasonalElements =
            "fall foliage, colorful autumn leaves, chrysanthemums, harvest colors, golden tones";
        } else {
          // December, January, February - Winter
          season = "winter";
          seasonalElements =
            "winter garden, evergreen plants, frost-covered leaves, winter flowers, hellebores, winter gardening prep";
        }
  ...
  const isPlaceholderTitle =
    templatePlaceholders.includes(block.headline || "") ||
  ...
  const isActivelyLoading =
    (block as any).isLoadingContent === true || isGenerating;
  ...
  return (
    ...
    >
      ...
      {isGenerating &&
        ...
        !(block as any).hasGeneratedContent &&
        !contentShownRef.current && (
          <BlockGeneratingOverlay message="Creating content for this section..." />
        )}
      {isEmpty && (
      ...
      {!isEmpty && (
        ...
        >
          ...
          {isTextBlockWithImage && (
            ...
            >
              {(() => {
                const isImageLoadingState =
                  block.imageUrl === "loading" ||
                  (block as any).isLoadingImage === true;
                const isBlockGeneratingImage =
                  (block as any).isGeneratingImage === true;

                if (
                  isGeneratingImage ||
                  isBlockGeneratingImage ||
                  isImageLoadingState ||
                  isAutoPickGenerating
                ) {
                  return (
            ...
            {hasContentLoaded ? (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground/70">
                  Click here to add the heading and content of the block
                </p>
              </div>
            )}
          </div>
            ...
            >
              {(() => {
                ...
                return displayImageUrl ? (
                    ...
                    {displayImageUrl && displayImageUrl !== "loading" && (
                      <img
                        ...
                        onError={() => {
                          if (displayImageUrl === block.imageUrl) {
                            console.error(
                              "[ImageTextBlock] Image failed to load:",
                              displayImageUrl,
                            );
                            setIsImageLoading(false);
                            setHasImageLoaded(false);
                  ...
                  <div className="w-full h-48 bg-muted rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-muted/80 transition-colors p-4 animate-gentle-pulse">
                    {isImageLoading ? (
                      <ImageSkeleton className="w-full h-full" />
                    ) : (
                      <>
