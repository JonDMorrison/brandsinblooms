...
export const TextBlock: React.FC<TextBlockProps> = ({
  ...
  const handleAutoPick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const blockContent =
        block.content || block.body || block.headline || block.title || "";
      const hasValidContent =
        typeof blockContent === "string" && blockContent.trim().length > 0;
  ...
  const hasContent =
    block.title || block.content || block.body || block.headline;
  const hasRichContent =
    hasContent &&
    (block.title || block.content || block.body || block.headline)!.length > 50;
  const isFromTemplate =
    block.source === "template" || block.source === "newsletter";

  // Show suggestion for rich content OR template-based blocks (regardless of length)
  const showAddImageSuggestion =
    (hasRichContent || isFromTemplate) &&
    !block.imageUrl &&
    isPreview &&
    onUpdate;
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground">
                    💡 This content would look great with an image
                  </span>
                </div>
                <Button
                  size="sm"
