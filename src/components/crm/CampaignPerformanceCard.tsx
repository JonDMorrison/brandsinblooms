...
> = ({ campaignName, sentDate, status, metrics, onViewDetails }) => {
  const calculateRate = (numerator: number, denominator: number): number => {
    if (denominator === 0) return 0;
    return Math.round((numerator / denominator) * 100);
  };

  const openRate = metrics
    ? calculateRate(metrics.adjustedOpens, metrics.delivered || metrics.sent)
    : 0;
  const clickRate = metrics
    ? calculateRate(metrics.clicked, metrics.delivered || metrics.sent)
    : 0;
  const deliveryRate = metrics
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                ...
                <div className="text-xl font-bold">
                  {metrics.adjustedOpens.toLocaleString()}
                </div>
              ...
              {metrics.revenueAttribution.revenue > 0 && (
                <div className="text-center">
                  <div className="flex items-center justify-center gap-1 text-sm text-muted-foreground mb-1">
                  ...
                  <div className="text-xl font-bold">
                    {formatAttributedRevenue(metrics.revenueAttribution)}
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Excludes machine-detected opens; clicks are more reliable.
                </p>
              </div>
            ...
            {(metrics.bounced > 0 ||
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 border-t">
                ...
                {(metrics.pending > 0 || metrics.unconfirmed > 0) && (
                  <div className="text-center">
                    <div className="text-sm text-muted-foreground mb-1">
                      Pending / unconfirmed
                    </div>
                    <div className="text-lg font-semibold text-amber-700">
                      {(metrics.pending + metrics.unconfirmed).toLocaleString()}
                    </div>
                  </div>
                )}
              </div>
