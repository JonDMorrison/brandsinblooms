...
export type SegmentViewFilter = "all" | "system" | "custom";
export type SegmentSortOption = "members-desc" | "name-asc" | "newest";

interface SegmentsFilterBarProps {
  query: string;
  onQueryChange: (value: string) => void;
  view: SegmentViewFilter;
  onViewChange: (value: SegmentViewFilter) => void;
  sort: SegmentSortOption;
  onSortChange: (value: SegmentSortOption) => void;
  resultCount: number;
  totalCount: number;
  loading?: boolean;
    ...
    >
      <Stack direction="row" spacing={0.5} useFlexGap flexWrap="wrap">
        {VIEW_OPTIONS.map((option) => (
          <JoyButton
            ...
            color="neutral"
            onClick={() => onViewChange(option.value as SegmentViewFilter)}
          >
            {option.label}
          </JoyButton>
        ))}
      ...
      <Box sx={{ minWidth: { xs: "100%", md: 180 } }}>
        <JoySelect
          label="Sort"
          value={sort}
          onValueChange={(value) => onSortChange(value as SegmentSortOption)}
...
export function SegmentsFilterBar({
  query,
  onQueryChange,
  view,
  onViewChange,
        ...
        >
          {loading ? (
            ...
            <Typography level="body-xs" color="neutral">
              {resultCount} of {totalCount} segments
            </Typography>
          )}
          {hasActiveFilters ? (
            <JoyButton
