...
> = ({
  ...
  const [selectedImageRecordId, setSelectedImageRecordId] = useState<
    string | null
  >(null);
    ...
    for (const dbMsg of dbMessages) {
      ...
      if (dbMsg.messageType === "user_prompt") {
        ...
        const uiMessage: Message = {
          ...
          thinkingDuration:
            dbMsg.metadata?.thinking_duration ||
            dbMsg.metadata?.thinkingDuration,
        };
        uiMessages.push(uiMessage);
  ...
  const convertDBMessagesToUI = async (
    dbMessages: any[],
  ): Promise<Message[]> => {
    const uiMessages: Message[] = [];
    ...
    for (const dbMsg of dbMessages) {
      if (dbMsg.messageType === "user_prompt") {
        ...
        const images = await AIChatPersistenceService.loadImagesForMessage(
          dbMsg.id,
        );
  ...
  const streamThinkingText = async (
    prompt: string,
    ...
    try {
      ...
      const errorText =
        "⚠️ Unable to generate thinking text. Continuing with image generation...";
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === thinkingMessageId
            ? {
  ...
  const generateImages = async (prompt: string) => {
    ...
    try {
      ...
      const thinkingContent = await streamThinkingText(
        prompt,
        thinkingMessage.id,
      );
      ...
      const { data: enhanceData } = await supabase.functions.invoke(
      ...
      setMessages((prev) =>
        prev.filter((msg) => msg.id !== enhancingMessage.id),
      );
      ...
      const errorMessage: Message = {
        ...
        type: "assistant",
        content:
          "Sorry, I encountered an error generating images. Please try again with a different prompt.",
        timestamp: new Date(),
      };
          ...
          <div className="relative flex flex-col h-full">
            ...
            <DialogHeader className="px-6 py-4 border-b bg-background/80 backdrop-blur-sm relative">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Describe your vision and I'll create beautiful garden images for
                you
              </p>
            </DialogHeader>
                ...
                <div className="flex items-center justify-center h-full">
                  <div className="text-center space-y-4 animate-fade-in">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Making assistant ready for use.
                    </p>
                  </div>
                </div>
              )}
              ...
              {!isInitialLoad && messages.length === 0 && (
                <div className="flex items-center justify-center h-full">
                  <div className="text-center space-y-3 max-w-md animate-fade-in">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Tell me what kind of garden image you'd like, and I'll
                      generate multiple options for you to choose from.
                    </p>
