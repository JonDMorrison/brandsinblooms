...
export const UnsplashPicker: React.FC<UnsplashPickerProps> = ({
  ...
  const getDynamicQuery = () => {
    if (task || campaign) {
      return (
        extractDynamicQuery(task, campaign) || initialQuery || "garden center"
      );
    }
    ...
    async (
      ...
      try {
        const response = await fetch(
          `https://api.unsplash.com/search/photos?query=${encodeURIComponent(searchQuery)}&page=${pageNum}&per_page=30&orientation=squarish&order_by=relevant`,
          {
            headers: {
        ...
        if (!response.ok) {
          ...
          const fallbackImages = (data?.images || []).map((img: any) => ({
            ...
            user: {
              ...
              links: {
                html: `https://unsplash.com/@${img.photographer.toLowerCase().replace(/\s+/g, "")}`,
              },
            },
            width: 400,
            height: 400,
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden">
        ...
        <div className="space-y-4">
          ...
          <div ref={scrollRef} className="max-h-[50vh] overflow-y-auto">
            ...
            {!loading && images.length === 0 && !query && (
              <div className="text-center py-12 text-gray-500">
                ...
                <p className="text-sm">
                  Enter keywords to find the perfect image for your post
                </p>
              </div>
            )}
          </div>
          ...
          <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded-lg">
            ...
            <p className="mt-1">
              Photographer attribution will be automatically added to your post.
            </p>
          </div>
        </div>
      </DialogContent>
