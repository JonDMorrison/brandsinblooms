...
import {
  extractDynamicQuery,
  getEnhancedTopicForPostType,
} from "@/utils/dynamicImageSearch";
...
const getGardenCenterFallback = (postType: string): string => {
  ...
  return typeSpecificFallbacks[
    Math.floor(Math.random() * typeSpecificFallbacks.length)
  ];
};
  ...
  const fetchImages = async (forceRefresh = false) => {
    ...
    try {
      ...
      if (error) {
        ...
        const fallbackData = await supabase.functions.invoke(
          "fetch-unsplash-images",
          {
            body: {
        ...
        {!selectedDraft ? (
          <div className={styles.emptyState}>
            ...
            <p className="text-xs text-gray-500">
              Select a draft to see relevant garden center images
            </p>
          </div>
        ) : (
          <ImageGalleryGrid
