...
export const RegenerateNewsletterButton: React.FC = () => {
      ...
      if (result.success) {
        ...
        if (result.hasUnwantedContent) {
          toast({
            ...
            description:
              "Some irrelevant themes may still be present. Please review the generated content.",
            variant: "destructive",
          });
        }
      } else {
        toast({
          ...
          description:
            result.error || "Failed to regenerate newsletter content",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  return (
    ...
    >
      {isRegenerating
        ? "Regenerating..."
        : "Regenerate World Vegetarian Day Newsletter"}
    </JoyButton>
  );
};
