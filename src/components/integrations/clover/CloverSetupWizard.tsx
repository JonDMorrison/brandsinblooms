...
const ICON_MAP: Record<QuickAutomation["icon"], React.ComponentType<React.SVGProps<SVGSVGElement>>> = {
  gift: Gift,
  star: Star,
  cake: Cake,
  clock: Clock,
...
export const CloverSetupWizard = ({
  ...
  const startSync = async () => {
    ...
    try {
      ...
      try { await supabase.functions.invoke("clover-sync-customers"); } catch {}
      ...
      try { await supabase.functions.invoke("clover-sync-sales"); } catch {}
      setSyncProgress((prev) => ({
        ...prev,
      ...
      try { await supabase.functions.invoke("clover-sync-products"); } catch {}
      setSyncProgress((prev) => ({ ...prev, products: { ...prev.products, status: "complete" } }));

      await new Promise((resolve) => setTimeout(resolve, 2000));
      const actualCounts = await fetchActualCounts();

      setSyncResults({ customersCount: actualCounts.customers, salesCount: actualCounts.sales, productsCount: actualCounts.products });
      setSyncProgress({
      ...
      toast({ title: "Sync encountered issues", description: "Some data may have synced.", variant: "destructive" });
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const createSelectedAutomations = async () => {
    if (selectedAutomations.size === 0) { setCurrentStep("complete"); return; }
    ...
    <Stack spacing={3} sx={{ py: 1 }}>
      <Stack alignItems="center" spacing={1}>
        ...
        <Typography level="body-sm" textColor="text.tertiary">
          We're importing your customers, sales, and products from Clover.
        </Typography>
        {merchantName && (
      ...
      {(
        [
          ...
          { key: "sales" as const, label: "Sales History", Icon: ShoppingCart },
  ...
  const renderOverviewStep = () => (
    <Stack spacing={3} sx={{ py: 1 }}>
      <Stack alignItems="center" spacing={1}>
        ...
        <Typography level="body-sm" textColor="text.tertiary">Here's what we found in your Clover account.</Typography>
      </Stack>
  ...
  const renderAutomationsStep = () => (
    <Stack spacing={2.5} sx={{ py: 1 }}>
      <Stack alignItems="center" spacing={1}>
        ...
        <Typography level="body-sm" textColor="text.tertiary">Enable automated campaigns to engage your customers.</Typography>
      </Stack>
        ...
        {CLOVER_QUICK_AUTOMATIONS.map((automation) => {
          ...
          return (
            <Sheet
              ...
              color="neutral"
              sx={{
                borderRadius: "lg", p: 1.5, cursor: "pointer",
                border: "1px solid",
            ...
            >
              <Stack direction="row" spacing={1.5} alignItems="center">
                <Box sx={{
                  flexShrink: 0, width: 36, height: 36, borderRadius: "md",
                ...
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Typography level="body-xs" textColor="text.tertiary" noWrap>{automation.description}</Typography>
                </Box>
                <Checkbox
                  checked={isSelected}
      ...
      <Stack direction="row" justifyContent="space-between">
        <Button variant="plain" color="neutral" onClick={goBack}>Back</Button>
        <Button
          variant="solid"
          color="neutral"
  ...
  const renderCompleteStep = () => (
    <Stack spacing={3} sx={{ py: 1, textAlign: "center" }}>
      <Stack alignItems="center" spacing={1.5}>
        <Box sx={{ width: 68, height: 68, borderRadius: "xl", bgcolor: "success.softBg", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <Typography level="body-sm" textColor="text.tertiary">Your Clover integration is ready to go.</Typography>
      </Stack>
  ...
  return (
    <Modal open={open} onClose={() => {}}>
      ...
      >
        ...
        <DialogTitle sx={{ sr: "only" }}>Clover Setup Wizard</DialogTitle>
