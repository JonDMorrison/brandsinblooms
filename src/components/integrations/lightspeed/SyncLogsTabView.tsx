...
export function SyncLogsTabView({
  ...
  onRetrySync: (syncType: LightspeedSyncLogRow["normalizedSyncType"]) => void;
  onRefresh: () => void;
  ...
  const hasVisibleUntrackedActiveJob = rows.some(
    (job) =>
      ...
      (!realtimeActive || !trackedJobIds.includes(job.id)),
  );
    ...
    <DataTabCard>
      ...
      {!isLoading && !isFetching && rows.length === 0 ? (
        <DataTabEmptyState
          ...
          description="Sync activity will appear here after your first sync."
        />
      ) : null}
    </DataTabCard>
  );
