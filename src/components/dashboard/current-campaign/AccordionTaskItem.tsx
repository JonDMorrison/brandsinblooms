...
export const AccordionTaskItem = ({
  ...
  } = useTaskImages(normalizedTask?.id);
  const hasContent =
    normalizedTask.ai_output && normalizedTask.ai_output.trim() !== "";

  // Check if this is a structured newsletter using normalized data
  const isStructuredNewsletter =
  ...
  if (hasContent) {
    if (isStructuredNewsletter && normalizedTask.normalized) {
      ...
      cleanContent =
        normalizedTask.normalized.newsletter_md || normalizedTask.ai_output;
      previewText = truncateText(
        cleanContent.replace(/[#*]/g, "").replace(/\s+/g, " ").trim(),
        110,
        "…",
      );
    } else {
      ...
      cleanContent =
        normalizedTask.display_content ||
        cleanContentForDisplay(
