...
export const DashboardCard = ({
  ...
  return (
    <Card
      className={`relative overflow-hidden border border-gray-200/60 rounded-2xl transition-all duration-300 ease-out hover:shadow-[0px_8px_24px_rgba(34,197,94,0.15)] hover:-translate-y-1 active:scale-[0.98] active:shadow-[0px_2px_8px_rgba(0,0,0,0.08)] shadow-[0px_6px_20px_rgba(0,0,0,0.08)] card-interactive group ${getVariantClasses()} ${getAccentBorderColor()}`}
    >
      ...
      <CardContent className="relative p-7 z-10">
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            <div
              className={`${getIconClasses()} transition-transform duration-200 ${hasPendingAction ? "animate-pulse" : ""} text-primary`}
            >
              {displayIcon}
            </div>
            <div>
              ...
              {statusMessage && (
                <div className="flex items-center gap-2">
                  {getStatusIcon()}
                  <span className="text-xs text-black">{statusMessage}</span>
                </div>
              )}
            </div>
          </div>
