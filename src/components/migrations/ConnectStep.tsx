...
export const ConnectStep = ({
  allowedProviders = ["mailchimp", "klaviyo", "constant_contact"],
  onComplete,
}: ConnectStepProps) => {
  ...
  const hasActiveMailchimpImport = activeJobs.some(
    (job) => job.source_platform === "mailchimp" && job.status === "running",
  );
  const hasActiveKlaviyoImport = activeJobs.some(
    (job) => job.source_platform === "klaviyo" && job.status === "running",
  );
  const hasActiveConstantContactImport = activeJobs.some(
    (job) =>
    ...
    try {
      ...
      if (data.error) {
        ...
        toast({
          ...
          description:
            data.message || `Please configure ${provider} OAuth credentials`,
          variant: "destructive",
        });
        setStatus("error");
        currentProviderRef.current = null;
  ...
  const handleDisconnect = async (provider: Provider) => {
    ...
    try {
      ...
      toast({
        title: "Disconnected",
        description:
          data?.message || `Successfully disconnected from ${provider}`,
      });
    } catch (error: any) {
  ...
  const canProceed = allowedProviders.some((provider) => {
    if (provider === "mailchimp") return mailchimpStatus === "connected";
    if (provider === "klaviyo") return klaviyoStatus === "connected";
    return constantContactStatus === "connected";
  });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Connect to {formatProviderList(allowedProviders)} to begin importing
          your contacts, segments, and tags.
        </p>
      </div>

      <div
        className={`grid gap-6 ${allowedProviders.length === 2 ? "md:grid-cols-2" : "md:grid-cols-3"}`}
      >
        {/* Mailchimp */}
        {allowedProviders.includes("mailchimp") ? (
          <Card className="p-6">
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <h3 className="font-semibold text-lg">Mailchimp</h3>
                <p className="text-sm text-muted-foreground">
                  Import from Mailchimp lists
                </p>
            ...
            {mailchimpAccount && (
              <div className="mb-4 p-3 bg-muted rounded-lg">
                <p className="text-sm font-medium">
                  {mailchimpAccount.accountname || "Connected Account"}
                </p>
                <p className="text-xs text-muted-foreground">
                  {mailchimpAccount.login?.email}
                </p>
              </div>
            )}
        ...
        {/* Klaviyo */}
        {allowedProviders.includes("klaviyo") ? (
          <Card className="p-6">
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <h3 className="font-semibold text-lg">Klaviyo</h3>
                <p className="text-sm text-muted-foreground">
                  Import from Klaviyo lists
                </p>
              ...
              <div className="flex items-center gap-2">
                ...
                {klaviyoStatus === "connected" && !hasActiveKlaviyoImport && (
                  <CheckCircle className="w-5 h-5 text-green-600" />
                )}
                {klaviyoStatus === "error" && (
                  <XCircle className="w-5 h-5 text-destructive" />
            ...
            {klaviyoAccount && (
              <div className="mb-4 p-3 bg-muted rounded-lg">
                <p className="text-sm font-medium">
                  {klaviyoAccount.name || "Connected Account"}
                </p>
                <p className="text-xs text-muted-foreground">
                  {klaviyoAccount.contact_email}
                </p>
              </div>
            )}
        ...
        {/* Constant Contact */}
        {allowedProviders.includes("constant_contact") ? (
          <Card className="p-6">
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <h3 className="font-semibold text-lg">Constant Contact</h3>
                <p className="text-sm text-muted-foreground">
                  Import from Constant Contact lists
                </p>
            ...
            {constantContactAccount && (
              <div className="mb-4 p-3 bg-muted rounded-lg">
                <p className="text-sm font-medium">
                  {constantContactAccount.name ||
                    constantContactAccount.organization_name ||
                    "Connected Account"}
                </p>
                <p className="text-xs text-muted-foreground">
                  {constantContactAccount.email ||
                    constantContactAccount.contact_email}
                </p>
              </div>
            )}
            ...
            {constantContactStatus !== "connected" ? (
              <Button
                ...
                className="w-full"
              >
                {constantContactStatus === "connecting" && (
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                )}
