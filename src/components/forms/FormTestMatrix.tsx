...
interface TestCase {
  ...
  run: (form: Form, supabaseUrl: string) => Promise<{ passed: boolean; message: string }>;
}
...
const TEST_CASES: TestCase[] = [
  {
    ...
    description: 'Verifies that forms requiring email consent reject submissions without consent checkbox',
    category: 'consent',
    run: async (form, supabaseUrl) => {
      ...
      if (!response.ok && result.error?.includes('consent')) {
        return { passed: true, message: 'Correctly rejected submission without email consent' };
      }
      
      return { passed: false, message: 'Failed to reject submission without email consent' };
    }
  },
  {
    ...
    run: async (form, supabaseUrl) => {
      ...
      if (!response.ok && result.error?.includes('SMS consent')) {
        return { passed: true, message: 'Correctly rejected submission without SMS consent' };
      }
      
      return { passed: false, message: 'Failed to reject submission without SMS consent' };
    }
  },
  {
  ...
  },
  {
    ...
    run: async (form, supabaseUrl) => {
      ...
      const rateLimited = results.some(r => r.error?.toLowerCase().includes('rate limit'));
      
      if (rateLimited) {
        return { passed: true, message: 'Rate limiting correctly triggered after rapid submissions' };
      }
      
      return { passed: false, message: 'Rate limiting did not trigger after 6 rapid submissions' };
    }
  },
  {
    id: 'valid_submission',
    ...
    run: async (form, supabaseUrl) => {
      ...
      if (response.ok && result.success) {
        return { passed: true, message: `Submission accepted. Customer ID: ${result.customer_id?.slice(0, 8)}...` };
      }
  ...
  },
  {
    ...
    run: async (form, supabaseUrl) => {
      ...
      if (!response.ok && result.error?.includes('required')) {
        return { passed: true, message: 'Correctly rejected submission with missing required fields' };
      }
      
      if (response.ok) {
        return { passed: false, message: 'Submission without email was incorrectly accepted' };
      }
