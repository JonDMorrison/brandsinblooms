...
const cleanupExistingYAML = (content: string): string => {
  ...
  if (cleaned.includes("blocks title:")) {
    ...
    const blocksMatch = cleaned.match(
      /blocks title:([\s\S]*?)(?=extra_content_ideas|meta:|$)/,
    );
    if (blocksMatch) {
