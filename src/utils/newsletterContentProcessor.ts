...
import {
  fixMalformedNewsletter,
  validateNewsletterStructure,
  calculateReadingTime,
} from "./newsletterContentFixer";
...
export const processNewsletterContent = (
  content: string,
  ...
  const needsFixing =
    !validateNewsletterStructure(content) ||
    content.includes("blocks title:") ||
    content.includes("blocks: title:");
  ...
  if (parsedNewsletter) {
    ...
    if (!hasValidBlocks) {
      if (parsedNewsletter.newsletter_md) {
        finalBlocks = createBlocksFromMarkdownContent(
          parsedNewsletter.newsletter_md,
        );
      } else {
        // Fallback: create blocks from raw content
        finalBlocks = createBlocksFromRawContent(
          processedContent,
          campaignTitle,
        );
      }
    ...
    if (finalBlocks.length < 2) {
      finalBlocks = ensureMinimumBlocks(
        finalBlocks,
        parsedNewsletter.newsletter_md || processedContent,
        campaignTitle,
      );
    }
    ...
    finalBlocks = finalBlocks.map((block) => ({
      ...block,
      image_prompt: improveImagePrompt(
        block.image_prompt,
        block.title,
        parsedNewsletter.meta?.theme,
      ),
    }));
    ...
    return {
      ...
      meta: {
        reading_time:
          parsedNewsletter.meta?.reading_time ||
          calculateReadingTime(processedContent),
        theme:
          parsedNewsletter.meta?.theme ||
          extractThemeFromContent(processedContent, campaignTitle),
        week_focus:
          parsedNewsletter.meta?.week_focus ||
          campaignTitle ||
          "Garden Newsletter",
      },
      isStructured: true,
  ...
  const unstructuredSections = createSectionsFromUnstructuredContent(
    processedContent,
    campaignTitle,
  );
...
const createBlocksFromRawContent = (
  content: string,
  campaignTitle?: string,
): any[] => {
  const blocks: any[] = [];
  ...
  if (paragraphs.length >= 2) {
    paragraphs.slice(0, 4).forEach((paragraph, index) => {
      ...
      blocks.push({
        ...
        cta: "Learn More",
        link: "#",
        image_prompt: generateThemeSpecificImagePrompt(
          blockTitle,
          campaignTitle,
        ),
...
const ensureMinimumBlocks = (
  ...
  if (blocks.length === 1) {
    ...
    if (existingBlock.body && existingBlock.body.length > 200) {
      const sentences = existingBlock.body
        .split(/[.!?]+/)
        .filter((s) => s.trim());
...
const createBlockFromSection = (section: {
  title: string;
  content: string[];
  isMainHeader: boolean;
}): any => {
...
const createSectionsFromUnstructuredContent = (
  content: string,
  campaignTitle?: string,
): UnstructuredSection[] => {
    ...
    parts.forEach((part, index) => {
      ...
      if (sectionContent) {
        sections.push({
          ...
          title,
          content: sectionContent,
          image_prompt: generateImagePrompt(
            title,
            sectionContent,
            campaignTitle,
          ),
...
const extractSectionTitle = (line: string): string | null => {
  ...
  if (
    title.length < 100 &&
    (title.includes(":") || title.match(/^[A-Z]/) || title.length < 60)
  ) {
    return title;
  }
...
const detectContentDuplication = (
  newsletter_md: string,
  blocks: any[],
): boolean => {
...
const generateThemeSpecificImagePrompt = (
  title: string,
  theme?: string,
  content?: string,
): string => {
  ...
  return `${theme || "garden"} ${cleanTitle.split(" ").slice(0, 2).join(" ")} professional garden center`.replace(
    /\s+/g,
    " ",
  );
};
...
const improveImagePrompt = (
  ...
  const hasKeywordStuffing =
    originalPrompt.includes("garden center") &&
...
const extractThemeFromContent = (
  ...
  if (textToAnalyze.includes("fall") || textToAnalyze.includes("autumn"))
    return "Fall Care";
...
const extractContentFromYamlString = (yamlString: string): string | null => {
  try {
    ...
    const newsletterMdMatch = yamlString.match(
    ...
    const contentMatch = yamlString.match(
      /content:\s*\|\s*\n([\s\S]*?)(?=\n\w+:|$)/,
    );
    if (contentMatch) {
    ...
    const simpleContentMatch = yamlString.match(
      /content:\s*["']?(.*?)["']?(?=\n|$)/,
    );
...
export const convertNewsletterMarkdownToHtml = (markdown: string): string => {
  ...
  return (
    markdown
      ...
      .replace(
        /^# (.+)$/gm,
        '<h1 class="text-4xl font-bold text-slate-900 mb-6">$1</h1>',
      )
      .replace(
        /^## (.+)$/gm,
        '<h2 class="text-2xl font-semibold text-slate-900 mb-4 mt-8">$1</h2>',
      )
      .replace(
        /^### (.+)$/gm,
      ...
      .replace(
        /^\*(.+)\*$/gm,
        '<p class="text-lg italic text-slate-600 mb-6">$1</p>',
      )
      // Convert bold text
      .replace(
        /\*\*(.+?)\*\*/g,
