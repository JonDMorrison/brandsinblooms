...
export const enhancedNewsletterToCRM = async (
  ...
  const getCleanHeaderContent = (
    ...
    if (
      ...
      !isSystemInstructionalText(processed.meta.theme)
    ) {
    ...
    if (
      ...
      !isSystemInstructionalText(campaign.description)
    ) {
    ...
    if (firstParagraph && !isSystemInstructionalText(firstParagraph)) {
      return (
        firstParagraph.replace(/\*\*(.*?)\*\*/g, "$1").substring(0, 100) + "..."
      );
    }
  ...
  const convertToContentBlocks = (
    ...
    if (headerTitle) {
      const cleanContent = getCleanHeaderContent(
        processed,
        contentTask.campaigns,
        contentTask.holidays,
      );
...
const generateSegmentSuggestionsFromContent = (
  content: string,
  theme: string,
): string[] => {
  // Reuse logic from sendToCRM utility
  ...
  const currentSeason = getCurrentSeason();
  suggestions.push(
    `${currentSeason.charAt(0).toUpperCase() + currentSeason.slice(1)} Care`,
  );
