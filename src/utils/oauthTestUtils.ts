...
export const testOAuthSetup = async (): Promise<{
  ...
  try {
    ...
    const { data: configData, error: configError } =
      await supabase.functions.invoke("get-oauth-config");
    ...
    if (currentDomain.includes("localhost")) {
      errors.push(
        "Running on localhost - Facebook OAuth will not work. Use the Lovable preview domain instead.",
      );
    } else if (
      !currentDomain.includes("lovable.app") &&
      !currentDomain.includes("brandsinblooms.com")
    ) {
      warnings.push(
        `Unknown domain: ${currentDomain}. Make sure this is whitelisted in Facebook app settings.`,
      );
    }
    ...
    if (staleState) {
      warnings.push(
        "Found stale OAuth state in browser storage - this will be cleared on next connection attempt",
      );
    ...
    const { data: exchangeData, error: exchangeError } =
      await supabase.functions.invoke("exchange-oauth-code", {
    ...
    if (exchangeError?.message?.includes("Authorization header required")) {
      ...
      exchangeData?.error?.includes("Authorization code is required")
    ) {
      info.exchangeEndpointWorking = true;
    } else {
      warnings.push(
        "OAuth exchange endpoint response was unexpected - check logs",
      );
    }
    return {
    ...
    errors.push(
      `Test failed with exception: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
