...
export const cleanupDuplicateCampaigns = async (
  userId: string,
  weekNumber: number,
) => {
  try {
    ...
    for (const [title, titleCampaigns] of Object.entries(campaignsByTitle)) {
      ...
      const campaignsToDelete = titleCampaigns.filter(
        (c) => c.id !== bestForTitle.id,
      );
      ...
      for (const campaign of campaignsToDelete) {
        ...
        if (updateError) {
          console.error(
            "Error updating tasks for campaign:",
            campaign.id,
            updateError,
          );
        }
    ...
    if (bestCampaign) {
      try {
        const { data: cleanupResult } = await supabase.functions.invoke(
...
export const generateMeaningfulTheme = async (
  weekNumber: number,
  companyName?: string,
) => {
