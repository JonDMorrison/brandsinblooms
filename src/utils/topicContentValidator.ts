...
export function validateCampaignContent(
  ...
  const topicKeywords =
    TOPIC_KEYWORDS[normalizedTopic] || generateGenericKeywords(normalizedTopic);
  ...
  const requiredScore =
    (foundRequired.length / topicKeywords.required.length) * 0.7;
  const relatedScore =
    Math.min(foundRelated.length / topicKeywords.related.length, 1) * 0.2;
  const forbiddenPenalty = foundForbidden.length * 0.15;

  const confidence = Math.max(
    0,
    Math.min(1, requiredScore + relatedScore - forbiddenPenalty),
  );

  // Determine if valid (need at least 80% of required keywords and >60% confidence)
  const isValid =
    foundRequired.length >= Math.ceil(topicKeywords.required.length * 0.8) &&
    confidence > 0.6 &&
  ...
  const result: TopicValidationResult = {
    ...
    missingKeywords: topicKeywords.required.filter(
      (keyword) => !foundRequired.includes(keyword),
    ),
    suggestions,
  };
  return result;
...
function generateValidationSuggestions(
  ...
  const missingRequired = keywords.required.filter(
    (k) => !foundRequired.includes(k),
  );
  if (missingRequired.length > 0) {
    suggestions.push(
      `Include these essential keywords: ${missingRequired.join(", ")}`,
    );
  }

  // Low related keyword count
  if (foundRelated.length < 2) {
    const availableRelated = keywords.related.filter(
      (k) => !foundRelated.includes(k),
    );
    if (availableRelated.length > 0) {
      suggestions.push(
        `Add topic-specific details using: ${availableRelated.slice(0, 3).join(", ")}`,
      );
    }
  }
  ...
  if (foundForbidden.length > 0) {
    ...
    suggestions.push(
      "Focus specifically on the topic instead of general advice",
    );
  }
