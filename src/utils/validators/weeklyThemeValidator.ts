 ...
 */
export const validateAllBlocksHaveImages = (blocks: ContentBlock[]): ValidationResult => {
  const errors: string[] = [];
  ...
  blocks.forEach((block, index) => {
    ...
    if (!block.imageUrl && !block.backgroundImageUrl) {
      errors.push(`Block ${index + 1} ("${block.title || block.headline || 'Untitled'}") is missing an image`);
    }
    
    // Check for text-only blocks (deprecated for weekly themes)
    if (block.type === 'text') {
      warnings.push(`Block ${index + 1} is using deprecated 'text' type - should be 'image-text' for weekly themes`);
    }
  });
 ...
 */
export const hasNoTextOnlyBlocks = (blocks: ContentBlock[]): ValidationResult => {
  const errors: string[] = [];
  
  const textBlocks = blocks.filter(block => block.type === 'text');
  
  if (textBlocks.length > 0) {
    errors.push(`Found ${textBlocks.length} plain text blocks - weekly themes require all blocks to have images`);
    textBlocks.forEach((block, idx) => {
      errors.push(`  - Block: "${block.title || block.headline || 'Untitled'}"`);
    });
  }
 ...
 */
export const validateImageQueries = (blocks: ContentBlock[]): ValidationResult => {
  const warnings: string[] = [];
  ...
  blocks.forEach((block, index) => {
    ...
    if (!block.imageQuery && !block.imageUrl) {
      warnings.push(`Block ${index + 1} has no image query - may fail generation`);
    }
  });
 ...
 */
export const convertTextBlocksToImageText = (blocks: ContentBlock[]): ContentBlock[] => {
  return blocks.map(block => {
    if (block.type === 'text') {
      console.log(`🔄 [WeeklyThemeValidator] Converting text block to image-text: "${block.title || block.headline}"`);
...
export const enforceSeasonalImageRelevance = (
  ...
  blocks.forEach((block, index) => {
    ...
    if (!hasSeasonalContext && query) {
      warnings.push(`Block ${index + 1} image query may not be seasonally relevant: "${query}"`);
    }
  });
