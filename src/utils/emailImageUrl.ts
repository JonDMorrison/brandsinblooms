...
export function isEmailSafeImageUrl(url: string | undefined | null): boolean {
  ...
  for (const pattern of BLOCKED_URL_PATTERNS) {
    if (pattern.test(trimmedUrl)) {
      console.warn('[emailImageUrl] Blocked URL pattern detected:', { url: trimmedUrl, pattern: pattern.toString() });
      return false;
    }
  }
 ...
 */
export function normalizeSupabaseStorageUrl(url: string | undefined | null): string | null {
  if (!url) return null;
...
export function getEmailSafeImageUrl(
  ...
  if (url) {
    console.warn('[emailImageUrl] Unsafe image URL filtered out:', { 
 ...
 */
export function sanitizeBlockImageUrls(block: Record<string, any>): Record<string, any> {
  const result = { ...block };
  
  // Common image URL fields in email blocks
  const imageFields = ['imageUrl', 'backgroundImageUrl', 'logoUrl', 'thumbnailUrl', 'coverImage'];
  ...
  blocks.forEach((block, index) => {
    ...
    for (const field of imageFields) {
      ...
      if (url && !isEmailSafeImageUrl(url)) {
        console.warn(`[emailImageUrl] Block ${index} (${block.type || 'unknown'}) has unsafe ${field}:`, {
          url: url.substring(0, 100),
          blockId: block.id,
