...
export const parseNewsletterYAML = (
  yamlContent: string,
): StructuredNewsletter | null => {
  try {
...
const repairYAMLStructure = (content: string): string => {
  ...
  repaired = repaired
    ...
    .replace(
      /^(\s{2,})(type:|title:|body:|image_prompt:|alt_text:|cta:|link:)/gm,
      "$1  $2",
    );
  ...
  if (matches.length > 0) {
    for (let i = 0; i < matches.length; i++) {
      ...
      if (currentMatch[1] === "newsletter_md:") {
        ...
        if (firstLine.includes("|")) {
          ...
          if (contentAfterPipe) {
            ...
            result.newsletter_md = [contentAfterPipe, ...followingLines]
              .join("\n")
              .trim();
          } else {
      ...
      if (currentMatch[1] === "blocks:") {
        ...
        const blockSection = sectionContent.substring(
          sectionContent.indexOf("blocks:") + 7,
        );
      ...
      if (currentMatch[1] === "meta:") {
        ...
        const metaSection = sectionContent.substring(
          sectionContent.indexOf("meta:") + 5,
        );
...
const parseBlocksManually = (blockContent: string): NewsletterBlock[] => {
  ...
  if (blockItems.length === 0) {
    blockItems = blockContent
      .split(/(?=\s*title:\s*")/g)
      .filter((item) => item.trim() && item.includes("title:"));
  }
  ...
  if (blockItems.length === 0) {
    ...
    const titleMatches = Array.from(
      blockContent.matchAll(/title:\s*"([^"]*?)"/g),
    );
    const bodyMatches = Array.from(
      blockContent.matchAll(/body:\s*"([^"]*?)"/g),
    );
    const ctaMatches = Array.from(blockContent.matchAll(/cta:\s*"([^"]*?)"/g));
    const imageMatches = Array.from(
      blockContent.matchAll(/image_prompt:\s*"([^"]*?)"/g),
    );
    ...
    if (titleMatches.length > 0 && bodyMatches.length > 0) {
      ...
      for (let i = 0; i < maxBlocks; i++) {
        blocks.push({
          ...
          link: "#",
          image_prompt:
            imageMatches[i]?.[1] || `${titleMatches[i][1]} garden newsletter`,
...
export const extractNewsletterSections = (
  content: string,
): { header: string; sections: Array<{ title: string; content: string }> } => {
