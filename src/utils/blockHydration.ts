...
export function isEmptyBlock(block: Partial<ContentBlock>): boolean {
  const hasHeadline =
    !isEmptyContent(block.headline) || !isEmptyContent(block.title);
  ...
  const hasCta =
    !isEmptyContent(block.ctaText) || !isEmptyContent(block.buttonText);
...
export function shouldHydrate(block: Partial<ContentBlock>): boolean {
  ...
  if (isUserEdited(block)) {
    newsletterDebug.log(
      "hydration",
      `Block ${block.id}: shouldHydrate=false (user-edited)`,
    );
    return false;
  }
  ...
  if (isAIGenerated(block)) {
    newsletterDebug.log(
      "hydration",
      `Block ${block.id}: shouldHydrate=false (ai-generated)`,
    );
    return false;
  }
  ...
  newsletterDebug.log(
    "hydration",
    `Block ${block.id}: shouldHydrate=${empty} (isEmpty=${empty})`,
  );
...
export function safeSetContent(
  ...
  if (isUserEdited(block)) {
    newsletterDebug.log(
      "hydration",
      `safeSetContent: Block ${block.id} is user-edited, skipping update`,
    );
    return block;
  }
...
export function validateBlockState(block: ContentBlock): string[] {
  ...
  if (block.userEdited && block.status !== "user-edited") {
    warnings.push(
      `Block ${block.id}: userEdited=true but status="${block.status}"`,
    );
  }

  if (block.hasGeneratedContent && block.status === "empty") {
    warnings.push(
      `Block ${block.id}: hasGeneratedContent=true but status="empty"`,
    );
  }
  ...
  if (block.isGeneratingImage && (block.imageUrl || block.backgroundImageUrl)) {
    warnings.push(
      `Block ${block.id}: isGeneratingImage=true but already has image`,
    );
  }

  // Log warnings
  if (warnings.length > 0) {
    newsletterDebug.warn(
      "hydration",
      `Block state validation warnings for ${block.id}:`,
      warnings,
    );
  }
...
export function needsImageGeneration(block: ContentBlock): boolean {
  ...
  if (block.type === "image-gallery" || block.type === "product-gallery") {
  ...
  const hasImage = isHeader ? !!block.backgroundImageUrl : !!block.imageUrl;
