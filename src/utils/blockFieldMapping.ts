 ...
 */
export function getBlockField<T>(
  block: Partial<ContentBlock> | Record<string, any>,
 ...
 */
export function convertEmailBlockToContentBlock(
  emailBlock: EmailBlock,
): ContentBlock {
  newsletterDebug.log(
    "mapping",
    `Converting EmailBlock to ContentBlock: ${emailBlock.id}`,
  );

  const isHeader = isHeaderBlock({ block_type: emailBlock.block_type });

  // Extract content fields from both top-level and nested content object
  const content = emailBlock.content || {};

  const headline =
    (emailBlock as any).headline || content.headline || content.title || "";
  const body =
    (emailBlock as any).body || content.body || content.content || "";
  const ctaText =
    (emailBlock as any).ctaText ||
    content.ctaText ||
    content.buttonText ||
    emailBlock.cta_text ||
    "";
  const ctaUrl =
    (emailBlock as any).ctaUrl ||
    content.ctaUrl ||
    content.buttonUrl ||
    emailBlock.cta_url ||
    "";
  ...
  const imageUrl = isHeader
    ? undefined
    : (emailBlock as any).imageUrl ||
      content.imageUrl ||
      emailBlock.image_url ||
      undefined;
  const backgroundImageUrl = isHeader
    ? (emailBlock as any).backgroundImageUrl ||
...
export function normalizeBlockForSave(
  ...
  const endTimer = newsletterDebug.startTimer(
    "mapping",
    `normalizeBlockForSave(${block.id})`,
  );
 ...
 */
export function normalizeBlockFromDatabase(
  dbBlock: DatabaseBlock,
): ContentBlock {
  if (!dbBlock || !dbBlock.id) {
    console.warn(
      "normalizeBlockFromDatabase: received null/invalid block, returning fallback",
    );
    return {
  ...
  const endTimer = newsletterDebug.startTimer(
    "mapping",
    `normalizeBlockFromDatabase(${dbBlock.id})`,
  );

  const contentObj = unwrapContentObject(dbBlock.content);
  const isHeader = isHeaderBlock({ block_type: dbBlock.block_type });

  newsletterDebug.log(
    "load",
    `Loading block ${dbBlock.id} (${dbBlock.block_type})`,
    {
  ...
  const headline =
    contentObj.headline || contentObj.title || contentObj.heading || "";
  ...
  const ctaText =
    dbBlock.cta_text || contentObj.ctaText || contentObj.buttonText || "";
  const ctaUrl =
    dbBlock.cta_url || contentObj.ctaUrl || contentObj.buttonUrl || "";
  ...
  const imageUrl = isHeader
    ? undefined
    : dbBlock.image_url || contentObj.imageUrl || undefined;
  const backgroundImageUrl = isHeader
  ...
  const hasGeneratedContent =
    contentObj.hasGeneratedContent === true ||
  ...
  const result: ContentBlock = {
    ...
    darkOverlayOpacity:
      dbBlock.dark_overlay_opacity ?? contentObj.darkOverlayOpacity,
...
export function logBlockState(block: ContentBlock, context: string): void {
  newsletterDebug.log(
    "hydration",
    `[${context}] Block ${block.id} (${block.type})`,
    {
      ...
      backgroundImageUrl:
        block.backgroundImageUrl?.substring(0, 50) || "(empty)",
