...
export const convertNewsletterToCRM = async (
  ...
  const { emailContent, blocks } = await convertToEmailBlocks(
    processed,
    preservedImages,
  );
...
const generateCampaignName = (title: string, processed: any): string => {
  ...
  if (
    processed.meta?.theme &&
    processed.meta.theme !== "Newsletter" &&
    processed.meta.theme !== "Garden Newsletter"
  ) {
...
const generateSubjectLine = (processed: any, title: string): string => {
  ...
  if (
    weekFocus &&
    weekFocus !== "Content Update" &&
    !weekFocus.toLowerCase().includes("newsletter")
  ) {
  ...
  if (
    processed.newsletter_md.toLowerCase().includes("fall") ||
    processed.newsletter_md.toLowerCase().includes("autumn")
  ) {
...
const convertToEmailBlocks = async (
  processed: any,
  originalImages?: Record<string, any>,
): Promise<{ emailContent: string; blocks: ContentBlock[] }> => {
  const blocks: ContentBlock[] = [];
  // Create header block from newsletter title and subtitle
  const headerBlock = await createHeaderBlock(
    processed,
    originalImages?.featured,
  );
  if (headerBlock) {
  ...
  if (processed.blocks && processed.blocks.length > 0) {
    ...
    const newsletterBlocks = processed.blocks.map(
    ...
    if (originalImages && Object.keys(originalImages).length > 0) {
      ...
      const imagePrompts = processed.blocks.map(
        (block: any) =>
          block.image_prompt ||
          block.alt_text ||
          block.title ||
          "garden center newsletter image",
      );
  ...
  if (
    processed.unstructuredSections &&
    processed.unstructuredSections.length > 0
  ) {
    const sectionBlocks = processed.unstructuredSections.map(
    ...
    if (originalImages && Object.keys(originalImages).length > 0) {
      ...
      // Extract image prompts for batch fetching
      const sectionImagePrompts = processed.unstructuredSections.map(
        (section: any) =>
...
`;
  ...
  if (processed.newsletter_md) {
    const emailFormattedContent = formatNewsletterForEmail(
...
export const buildEmailHtmlFromNewsletter = (
  content: string,
  title?: string,
): string => {
...
const formatNewsletterForEmail = (content: string): string => {
  ...
  formatted = formatted
    .replace(
      /^# (.+)$/gm,
      '<h2 style="color: #1e40af; font-size: 24px; font-weight: 600; margin: 0 0 20px 0; padding-bottom: 10px; border-bottom: 2px solid #e2e8f0;">$1</h2>',
    )
    .replace(
      /^## (.+)$/gm,
      '<h3 style="color: #475569; font-size: 20px; font-weight: 600; margin: 25px 0 15px 0;">$1</h3>',
    )
    .replace(
      /^### (.+)$/gm,
      '<h4 style="color: #64748b; font-size: 18px; font-weight: 500; margin: 20px 0 10px 0;">$1</h4>',
    );
  ...
  try {
    ...
    if (preservedFeaturedImage) {
      ...
      try {
        ...
        headerImage = {
          url: "/images/newsletter-fallback.jpg",
