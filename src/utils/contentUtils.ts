...
export const filterSupportedContent = <
  T extends { post_type: string; campaign_id?: string },
>(
  items: T[],
): T[] => {
  const filteredItems = items.filter((item) =>
    SUPPORTED_POST_TYPES.includes(item.post_type as SupportedPostType),
  );
...
export const isSupportedPostType = (
  postType: string,
): postType is SupportedPostType => {
...
export const parseNewsletterJson = (
  content: string,
...
const shouldAddBlogStructure = (text: string): boolean => {
  ...
  const hasExistingStructure =
    /^#{2,6}\s+/.test(text) || text.includes("<h") || text.includes("##");
...
export const formatBlogContent = (text: string): string => {
  ...
  formatted = formatted
    .replace(
      /^#{2}\s+(.+)$/gm,
      '<h2 class="text-3xl font-semibold font-display text-slate-900 mt-10 mb-4">$1</h2>',
    )
    .replace(
      /^#{3}\s+(.+)$/gm,
      '<h3 class="text-2xl font-semibold font-display text-slate-900 mt-8 mb-3">$1</h3>',
    )
    .replace(
      /^#{4}\s+(.+)$/gm,
      '<h4 class="text-xl font-semibold font-display text-slate-900 mt-6 mb-2">$1</h4>',
    )
    // Convert blockquotes with custom styling
    .replace(
      /^>\s+(.+)$/gm,
      '<blockquote class="border-l-4 border-primary bg-primary/5 pl-6 py-4 my-6 italic text-slate-700">$1</blockquote>',
    )
  ...
  formatted = formatted
    ...
    .replace(
      /\[!TIP\]\s*\n(.*?)(?=\n\n|\n$|$)/gs,
      '<aside class="bg-primary/10 border-l-4 border-primary rounded-lg p-4 my-8"><p class="m-0 text-slate-700">💡 $1</p></aside>',
    )
    .replace(
      /\[!NOTE\]\s*\n(.*?)(?=\n\n|\n$|$)/gs,
      '<aside class="bg-blue-50 border-l-4 border-blue-400 rounded-lg p-4 my-8"><p class="m-0 text-slate-700">📝 $1</p></aside>',
    )
    .replace(
      /\[!WARNING\]\s*\n(.*?)(?=\n\n|\n$|$)/gs,
      '<aside class="bg-orange-50 border-l-4 border-orange-400 rounded-lg p-4 my-8"><p class="m-0 text-slate-700">⚠️ $1</p></aside>',
    )
    .trim();
  return formatted;
};
...
export const cleanContentForDisplay = (
  content: string,
  postType: string = "",
): string => {
  ...
  try {
    ...
    if (cleanContent.includes("newsletter_md:")) {
      ...
      for (const line of lines) {
        if (
          line.trim() === "newsletter_md: |" ||
          line.trim().startsWith("newsletter_md: |")
        ) {
          inNewsletterMd = true;
...
const cleanNewsletterForDisplay = (content: string): string => {
  ...
  if (parsedNewsletter) {
    ...
    const cleanSubject = parsedNewsletter.subject
      ? stripAllNewsletterFormatting(parsedNewsletter.subject)
      : "";
    const cleanContent = parsedNewsletter.content
      ? stripAllNewsletterFormatting(parsedNewsletter.content)
      : "";
