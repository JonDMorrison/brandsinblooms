...
import {
  convertNewsletterToCRM,
  createCRMCampaignFromNewsletter,
} from "./newsletterToCrmSync";
...
export const sendToCRM = async (contentTaskId: string): Promise<boolean> => {
  try {
    ...
    if (isNewsletterContent) {
      const conversionResult = convertNewsletterToCRM(
    ...
    if (isNewsletterContent) {
      ...
      const searchParams = new URLSearchParams({
        ...
        ...(segmentSuggestions?.length && {
          segmentSuggestions: encodeURIComponent(
            JSON.stringify(segmentSuggestions),
          ),
        }),
        ...
        ...(newsletterBlocks.length && {
          newsletterBlocks: encodeURIComponent(
            JSON.stringify(newsletterBlocks),
          ),
        }),
      });
...
const extractPersonaTags = (content: string): string[] => {
  ...
  if (
    lowerContent.includes("vegetable") ||
    lowerContent.includes("herb") ||
  ...
  if (
    lowerContent.includes("flower") ||
    lowerContent.includes("bloom") ||
    lowerContent.includes("rose")
  ) {
    tags.push("Flower Enthusiasts");
  }

  if (
    lowerContent.includes("indoor") ||
    lowerContent.includes("houseplant") ||
    lowerContent.includes("container")
  ) {
...
const generateSegmentSuggestions = (
  ...
  if (currentMonth >= 2 && currentMonth <= 4) {
    ...
    if (
      ...
      lowerContent.includes("protection")
    ) {
...
const extractImages = (contentTask: any): string[] => {
  ...
  if (contentTask.attachments) {
    const attachments =
      typeof contentTask.attachments === "string"
