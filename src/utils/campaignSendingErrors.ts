...
export const SEND_ERRORS: Record<SendErrorCode, Omit<SendError, "code">> = {
  ...
  NO_SEGMENTS: {
    ...
    description:
      "Campaign has no segments selected. Please select an audience.",
  ...
  NO_CONTENT: {
    ...
    description:
      "Your email has no content blocks. Please add content before sending.",
    recoverable: true,
  },
  NO_OPTED_IN_CUSTOMERS: {
    title: "No eligible recipients",
    description:
      "There are no eligible opted-in contacts to email after BloomSuite applied consent and suppression checks.",
    action:
      "Review your audience, import contacts with consent, or check skipped contacts for details.",
    recoverable: true,
  },
  EMAIL_SERVICE_NOT_CONFIGURED: {
  ...
  SENDER_DOMAIN_REQUIRED: {
    ...
    description:
      "Campaign sending requires a configured custom sending domain.",
    action:
      "Go to Email Settings to configure and verify your sending domain, then try again.",
    recoverable: true,
  },
  DOMAIN_AUTHENTICATION_REQUIRED: {
    title: "Email authentication required",
    description:
      "This sending domain is missing one or more required authentication checks.",
    action:
      "Verify SPF, DKIM, return-path, DMARC, and domain ownership in Email Settings, then try again.",
    recoverable: true,
  },
  QUOTA_EXCEEDED: {
    title: "Sending limit reached",
    description:
      "This campaign is larger than the current sending limit for this account or domain.",
    action:
      "Try a smaller audience or contact support to review the sending limit.",
    recoverable: true,
  },
  DOMAIN_NON_COMPLIANT_FOR_SCALE: {
    title: "Review needed before high-volume send",
    description:
      "This campaign is large enough that BloomSuite needs a few sender details cleaned up before sending safely.",
    action:
      "Fix the items shown in Email Settings or reduce the audience size and send again.",
    recoverable: true,
  },
  DOMAIN_BLOCKED: {
    title: "Sending paused for this domain",
    description:
      "This sending domain is paused or blocked and needs support review before more campaigns can send.",
  ...
  PROTECTED_SEND_ACTIVE: {
    ...
    description:
      "BloomSuite will send this campaign more gradually to protect inbox placement.",
    action:
      "No action is required. The campaign can continue with protected pacing.",
    recoverable: true,
  },
  SEND_PAUSED_BY_ADMIN: {
    title: "Campaign sending is paused",
    description:
      "This campaign was paused by an admin or support rule before sending.",
    action:
      "Review the campaign status or contact support if you believe this is incorrect.",
    recoverable: true,
  },
  HIGH_VOLUME_REVIEW_REQUIRED: {
    title: "Review needed before high-volume send",
    description:
      "This high-volume campaign needs a few deliverability checks resolved before sending.",
    action:
      "Review the highlighted sender, content, or business-address items, then try again.",
    recoverable: true,
  },
  CAMPAIGN_SAVE_FAILED: {
...
export function parseEdgeFunctionError(
  ...
  const reputationAction =
    typeof data?.reputation?.action === "string" ? data.reputation.action : "";
  ...
  if (
    ...
    return {
      ...
      ...SEND_ERRORS.EMAIL_SERVICE_NOT_CONFIGURED,
    };
  }
  ...
  if (
    ...
    return {
      code: "SENDER_DOMAIN_REQUIRED",
      ...SEND_ERRORS.SENDER_DOMAIN_REQUIRED,
    };
  }
  ...
  if (
    ...
    message.includes("Campaign is paused")
  ) {
    return {
  ...
  if (
    ...
    return {
      ...
      ...SEND_ERRORS.SEND_PAUSED_BY_ADMIN,
      description:
        bodyError || message || SEND_ERRORS.SEND_PAUSED_BY_ADMIN.description,
    };
  }
  ...
  if (
    ...
    return {
      code: "PROTECTED_SEND_ACTIVE",
      ...SEND_ERRORS.PROTECTED_SEND_ACTIVE,
    };
  }

  if (
    message.includes("no segments selected") ||
    message.includes("Campaign has no segments")
  ) {
    return { code: "NO_SEGMENTS", ...SEND_ERRORS.NO_SEGMENTS };
  }

  if (
    message.includes("No customers found") ||
    ...
    return {
      code: "NO_OPTED_IN_CUSTOMERS",
      ...SEND_ERRORS.NO_OPTED_IN_CUSTOMERS,
    };
  }
  ...
  if (
    ...
    bodyError.includes("requires verified SPF, DKIM, return-path, DMARC") ||
    message.includes("requires verified SPF, DKIM, return-path, DMARC")
  ) {
    return {
      code: "DOMAIN_AUTHENTICATION_REQUIRED",
      ...SEND_ERRORS.DOMAIN_AUTHENTICATION_REQUIRED,
    };
  }
  ...
  if (
    ...
    return {
      code: "HIGH_VOLUME_REVIEW_REQUIRED",
      ...SEND_ERRORS.HIGH_VOLUME_REVIEW_REQUIRED,
    };
  }
...
export function validateBeforeSend(params: {
  ...
  if (!campaignName?.trim()) {
    return {
      valid: false,
      error: { code: "NO_CAMPAIGN_NAME", ...SEND_ERRORS.NO_CAMPAIGN_NAME },
    };
  }
  ...
  if (!subjectLine?.trim()) {
    return {
      valid: false,
      error: { code: "NO_SUBJECT_LINE", ...SEND_ERRORS.NO_SUBJECT_LINE },
    };
  }
  ...
  if (!hasBlocks && !hasContent) {
    return {
      valid: false,
      error: { code: "NO_CONTENT", ...SEND_ERRORS.NO_CONTENT },
    };
  }
