...
export const DashboardProvider = ({ children }: DashboardProviderProps) => {
  ...
  const [composerMode, setComposerMode] = useState<"draft" | "scheduled">(
    "draft",
  );
  ...
  const scheduleDraftCallback = useCallback(
    async (draftId: string, dateStr: string) => {
  ...
  const handleClickToPost = useCallback(
    async (task: any) => {
      ...
      try {
        ...
        const { bestDateTime } = await getBestSlot(
  ...
  useEffect(() => {
    if (data?.tasks) {
      ...
      const availableDrafts = data.tasks
        ...
        .sort((a: any, b: any) => {
          if (
            a.status === TASK_STATUS.APPROVED &&
            b.status !== TASK_STATUS.APPROVED
          )
            return -1;
          if (
            b.status === TASK_STATUS.APPROVED &&
      ...
      if (draftOrder.length === 0) {
        ...
        const removedTasks = draftOrder.filter(
  ...
  const getOrderedDrafts = useCallback(() => {
    ...
    const availableTasks = data.tasks.filter((task: any) =>
      visibleStatuses.includes(task.status as TaskStatus),
    );
    ...
    return draftOrder.map((id) => taskMap.get(id)).filter(Boolean); // Remove any undefined entries
