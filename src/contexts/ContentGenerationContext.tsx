...
const ContentGenerationContext = createContext<
  ContentGenerationContextType | undefined
>(undefined);
...
export const useContentGeneration = () => {
  ...
  if (!context) {
    throw new Error(
      "useContentGeneration must be used within a ContentGenerationProvider",
    );
  }
  return context;
};
...
export const ContentGenerationProvider = ({
  children,
}: ContentGenerationProviderProps) => {
  const { user } = useAuth();
  const { tenant } = useTenant();
  const [generatingCampaigns, setGeneratingCampaigns] = useState<Set<string>>(
    new Set(),
  );
    ...
    async (
      ...
      try {
        ...
        if (result.success && result.tasks) {
          ...
          const tasksNeedingImages = result.tasks.filter(
            (task: any) => task.imageQuery && !task.image_url,
          );
          ...
          if (tasksNeedingImages.length > 0) {
            ...
            import("@/integrations/supabase/client").then(({ supabase }) => {
              ...
              supabase.functions
                .invoke("fetch-and-assign-images", {
                  body: { tasks: imageTasks },
                })
                .then(({ data, error }) => {
                  if (error) {
                    console.error(
                      "[ContentGeneration] Image fetch error:",
                      error,
                    );
                  } else if (data) {
                  }
                ...
                .catch((err) => {
                  console.error(
                    "[ContentGeneration] Image fetch exception:",
                    err,
                  );
                });
            });
  ...
  const isGeneratingForCampaign = useCallback(
    (campaignId: string) => {
